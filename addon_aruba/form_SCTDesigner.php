 ...
 // $login_qualify = 'ACT';
  ...
  if(isset($_POST['fieldid']))
  {
	...
	{ // color picked, convert to style and set in database
	  ...
	  mysql_query("UPDATE ". $SCTprefix. "SCTtest SET testcolor='". $colordata. "' WHERE sctid=". $_POST['ccid'], $userlink);
	}
  ...
  { // Select an SCT test or create a new one
    $sctlist = SCTtest::listObjects();
		if(isset($sctlist))
		{
			...
			{
				echo("<TR><TD style='". $scttestobj->get_style(). "'>". $scttestobj->get_description(). "</td><td><a href=". $_SERVER['PHP_SELF']. "?edit=". $scttestobj->get_id(). "><img src='PNG/reply.png'></a></td><td><a href=". $_SERVER['PHP_SELF']. "?dtest=". $scttestobj->get_id(). "><img src='PNG/action_delete.png'></a></td>");
			}
			echo("</table>");
		}
  ...
  { // Editing a selected SCT test
    // 0 normally indicates a new one but if an id is set for refresh we need to revise that
		...
		$scttestobj->edit_description();
		  echo(" Klik op de volgende tekst om de kleur aan te passen: <SPAN style='". $scttestobj->get_style(). "' onClick=openpicker(". $scttestobj->get_id(). ")>". $scttestobj->get_description(). "</SPAN>");
	...
	echo("<FORM METHOD=POST ID=pickform><input type=hidden name=ccid id=ccid><input type=hidden name=bgcol id=bgcol><input type=hidden name=fgcol id=fgcol></FORM>");
