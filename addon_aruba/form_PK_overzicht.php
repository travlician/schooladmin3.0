...
//
  ...
  $houdingdata = SA_loadquery("SELECT aspect,sid,xstatus,COUNT(houdingid) AS score FROM avo_pk_data WHERE year='". $curyear. "' AND period=". $CurrentPer. " GROUP BY sid,aspect,xstatus");
  ...
  echo("<form method=post action=form_PK_overzicht.php ID=groupsel NAME=groupsel>" . $dtext['Group_Cap'] . 
...
/*  echo("<form method=post action=form_PK_overzicht.php ID=subjsel NAME=subjsel>" . $dtext['Subject'] . 
  ...
  echo("<form method=post action=form_PK_overzicht.php ID=persel NAME=persel>" . $dtext['Period'] . 
  ...
  foreach($aspects AS $aspkey => $aspect)
  {
    ...
    echo("<td colspan=4><center><SPAN title='". $aspect. "'>". $aspkey. "</span></td>");
  }
  echo("</tr>");
...
/*  echo("<tr><td><center>Leerling</td>");
  ...
  foreach($aspects AS $aspect)
  {
    echo("<th class=per". $CurrentPer. "><center>G</td>");
    echo("<th class=per". $CurrentPer. "><center>V</td>");
    echo("<th class=per". $CurrentPer. "><center>M</td>");
    echo("<th class=per". $CurrentPer. "><center>S</td>");
  }
  echo("</tr>");
  ...
  foreach($students AS $stobj)
  {
   if(isset($stobj))
   {
	...
	foreach($aspects AS $aspix => $aspect)
	{
	  ...
	  echo("<TD class=per". $CurrentPer. ">". (isset($hdata[$sid][$aspix]["G"]) ? ROUND($hdata[$sid][$aspix]["G"] * 100.0 / $totacnt,0). "%" : " "). "</td>");
	  echo("<TD class=per". $CurrentPer. ">". (isset($hdata[$sid][$aspix]["V"]) ? ROUND($hdata[$sid][$aspix]["V"] * 100.0 / $totacnt,0). "%" : " "). "</td>");
	  echo("<TD class=per". $CurrentPer. ">". (isset($hdata[$sid][$aspix]["M"]) ? ROUND($hdata[$sid][$aspix]["M"] * 100.0 / $totacnt,0). "%" : " "). "</td>");
	  echo("<TD class=per". $CurrentPer. ">". (isset($hdata[$sid][$aspix]["S"]) ? ROUND($hdata[$sid][$aspix]["S"] * 100.0 / $totacnt,0). "%" : " "). "</td>");
	}
	echo("</tr>");
   }
  }
