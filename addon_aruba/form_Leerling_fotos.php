...
//  $imglocs = SA_loadquery("SELECT sid,xoff,yoff FROM student_imgloc LEFT JOIN sgrouplink USING(sid) LEFT JOIN sgroup USING(gid)
//                           WHERE active=1 AND tid=tid_mentor AND groupname= '$CurrentGroup'");
...
//  $imglocs = SA_loadquery("SELECT sid,xoff,yoff FROM student_imgloc LEFT JOIN sgrouplink USING(sid) LEFT JOIN sgroup USING(gid)
//                           WHERE active=1 AND tid=". $uid. " AND groupname= '$CurrentGroup'");
  if(isset($imglocs['sid']))
  {
    foreach($imglocs['sid'] AS $six => $sid)
	{
  ...
  // We need some mentor data, so get that
  $mygrp = new group();
  $mygrp->load_current();
  $mentor = $mygrp->get_mentor();
  if(isset($mentor) && isset($teachercode))
  {
    $tcode = $mentor->get_teacher_detail($teachercode);
  }

  echo '<LINK rel="stylesheet" type="text/css" href="style.css" title="style1">';
  echo("<H2 style='text-align: center'><IMG src=schoollogo.png> Klas ". $_SESSION['CurrentGroup']. "&nbsp;&nbsp;&nbsp;&nbsp;". $curyear. (isset($tcode) ? "&nbsp;&nbsp;&nbsp;&nbsp;Mentor: ". $tcode : ""). "</H2>");
?>
<style>
.dragme
{
...
<?
  ...
  {
    if($students['imgname'][$six] != "")
      echo("<IMG SRC='". $livepictures. $students['imgname'][$six]. 
	     "' class=dragme TITLE='". $students['name'][$six]. "' ID=". $sid. ">");
	else
      echo("<SPAN style='display: inline-block; width: 100px; text-align: center; margin: 1px 10px; vertical-align: top;'>". $curyear. "<IMG SRC='schoollogo.png' style='width: 100px;' TITLE='". $students['name'][$six]. "' ID=". $sid. ">". $students['name'][$six]. "</SPAN>");
    $poscnt++;
	if($poscnt % 5 == 0)
	  echo("</p><p style='width: 900px;'>");
  }
  
  // See if we can get a mentor image
  if(isset($mentor))
  {
    $tdets = inputclassbase::load_query("SELECT table_name FROM teacher_details WHERE type='picture' ORDER BY seq_no");
	if(isset($tdets['table_name'][0]))
	{
	  $timg = $mentor->get_teacher_detail($tdets['table_name'][0]);
	  if(isset($timg))
	  {
	    $timg = substr($timg,0,strpos($timg,"WIDTH")). " WIDTH=100 HEIGHT=140>";
	    echo("<SPAN style='display: inline-block; width: 100px; height: 140px; max-height: 140px; margin: 1px 10px; padding-left:". (480 - ($poscnt % 5) * 120). "px;'>". $timg. "</SPAN>");
	  }
	}
  }
