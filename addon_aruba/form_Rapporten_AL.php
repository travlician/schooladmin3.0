<?php
  ...
  $mainsubjectabbrevs = array( 1 => "vk","en","sp","lo","hv","te","mu","god","ne","le","re","sch","ak","ges","kdn","ml");
  $subsubjects["ne"] = array(1 => "Dictee", "Taaloefening","Tekst");
  $subsubjectabbrevs["ne"] = array(1 => "dic","ta-oef","txt");
  ...
  $aspects = array('Gedr' => 'Gedrag', 'Conc' => 'Concentratie', 'Wrkt' => 'Werktempo', 'Zelfs' => 'Zelfstandigheid',
                             'SocVrd' => 'Sociale vaardigheden', 'Wrkvz' => 'Werkverzorging', 'MotIjvr' => 'Motivatie/ijver');
  ...
  */
  ...
  $absq1 = "SELECT sid,SUM(IF(acid=1,1,0)) AS absent, SUM(IF(acid=2,1,0)) AS late, SUM(IF(acid=3,1,0)) AS homework FROM absence LEFT JOIN absencereasons USING(aid) 
            WHERE date >= '". $perstart[1]. "' AND date <= '". $perend[1]. "' GROUP BY sid";
  ...
  $absq2 = "SELECT sid,SUM(IF(acid=1,1,0)) AS absent, SUM(IF(acid=2,1,0)) AS late, SUM(IF(acid=3,1,0)) AS homework FROM absence LEFT JOIN absencereasons USING(aid) 
            WHERE date >= '". $perstart[2]. "' AND date <= '". $perend[2]. "' GROUP BY sid";
  ...
  $absq3 = "SELECT sid,SUM(IF(acid=1,1,0)) AS absent, SUM(IF(acid=2,1,0)) AS late, SUM(IF(acid=3,1,0)) AS homework FROM absence LEFT JOIN absencereasons USING(aid) 
            WHERE date >= '". $perstart[3]. "' AND date <= '". $perend[3]. "' GROUP BY sid";
  ...
  echo(">");
  echo '<LINK rel="stylesheet" type="text/css" href="style_rapport_AL.css" title="style1">';
  
  foreach($student_array['sid'] AS $si => $sidx)
  {
    // Header: Logo left and school info text right
	echo("<DIV class=schoollogo><IMG src=schoollogo.png border=0 width=100></DIV>");
	echo("<DIV class=rapportheader>RAPPORT</DIV>");
	echo("<DIV class=schoolinfo>St. Aloysius School<BR>Noord 8<BR>Tel.: 587-2364<BR>Fax.: 587-2364</DIV>");
	...
	echo("<TR><TH>&nbsp;</th><TH>R1</th><TH>R2</th><TH>R3</th>");
	...
	echo("</TR></TABLE>");
	if(isset($rapopms[$sidx]))
	{
	  echo("<P class=remarkheader>Opmerkingen:</P>");
	  echo("<P class=remarkblock>");
	  for($per=1; $per <=3; $per++)
	  {
	    if(isset($rapopms[$sidx][$per]))
		  echo("<b>Rapport ". $per. " :</b><BR>". $rapopms[$sidx][$per]. "<BR>");
	  }
	  echo("</P>");
	}
	if(isset($yradvice[$sidx]) && $yradvice[$sidx] != "")
	{
	  echo("<P class=adviceheader>Advies:</P>");
	  echo("<P class=adviceblock>". $yradvice[$sidx]. "</P>");
	}

	echo("<P class=clearfloats>Betekenis van de letters:</P>");
	...
	echo("<TR><TD>Hoofd:</TD><TD>dhr. Vladimir de Cuba</TD><TD>________________________________________</TD></TR>");
  ...
  function show_subject($sbix)
  {
	...
	//if(isset($mid) && isset($result[$sidx][$mid][0]) && $curperiod >= 3 && $subjects4period[$sbix] == 1)
	{ // There is a period 3 result
	  show_result($result[$sidx][$mid][0]);
	}
	else if($subjects4period[$sbix] == 1 && substr($CurrentGroup,0,1) > 3)
	//else if($subjects4period[$sbix] == 1)
