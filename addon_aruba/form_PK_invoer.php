...
//
  ...
  $statquery = "SELECT '' AS id, ' ' AS tekst UNION SELECT 'G' AS id,'G' AS tekst UNION SELECT 'V','V' UNION SELECT 'M','M' UNION SELECT 'S','S'";
  ...
  echo("<form method=post action=form_PK_invoer.php ID=groupsel NAME=groupsel>" . $dtext['Group_Cap'] . 
  ...
  echo("<form method=post action=form_PK_invoer.php ID=subjsel NAME=subjsel>" . $dtext['Subject'] . 
	...
	foreach($aspects AS $aspix => $aspect)
	{
	  for($tper=1; $tper <= $percount; $tper++)
	  {
	    ...
	    { // Create an entry field for the aspect
		  ...
		  {
            $statfield = new inputclass_listfield("statfield". $hdata[$sid][$aspix][$tper],$statquery,$userlink,"xstatus","avo_pk_data",$hdata[$sid][$aspix][$tper],"houdingid","","hdprocpage.php");
		  }
		  else
		  {
