...
//
  ...
  // Get a list of all applicable students
	if(isset($_SESSION['ssortertable']) && $_SESSION['ssortertable'] != '' && $_SESSION['ssortertable'] != '-')
		$students = SA_loadquery("SELECT CONCAT(lastname,', ',firstname) AS name,firstname, lastname, sid FROM student LEFT JOIN sgrouplink USING(sid) LEFT JOIN sgroup USING(gid) LEFT JOIN ". $_SESSION['ssortertable']. " USING(sid) WHERE active=1 AND sgroup.groupname = '$CurrentGroup' ORDER BY data,name");
	else
