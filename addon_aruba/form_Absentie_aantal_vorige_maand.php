...
//
  ...
  $lday = date("j",mktime(0,0,0,date("n"),0,date("Y")));
  $daynames = array( 1 => "M","D","W","D","V");
  for($dt=1; $dt<= $lday; $dt++)
  {
    $wday = date("N",mktime(0,0,0,date("n")-1,$dt,date("Y")));
    if($wday < 6)
	  $dtab[$dt] = $daynames[$wday];
  }
  $dispmonth = date("n",mktime(0,0,0,date("n"),0,date("Y")));
  $dispyear = date("Y",mktime(0,0,0,date("n"),0,date("Y")));
