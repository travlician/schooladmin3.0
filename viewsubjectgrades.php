...
//
  ...
  $sql_query = "SELECT student.*,sgrouplink.gid FROM student LEFT JOIN sgrouplink USING(sid) LEFT JOIN sgroup USING(gid) WHERE active=1 AND sgroup.groupname='$CurrentGroup' ORDER BY lastname,firstname";
  ...
  echo("<html><head><title>" . $dtext['grsub_title'] . "</title></head><body background=schooladminbg.jpg link=blue vlink=blue>");
  echo '<LINK rel="stylesheet" type="text/css" href="style.css" title="style1">';
  echo("<font size=+2><center>" . $dtext['Grades4'] . " " . $subject_array['fullname'][1] . " " . $dtext['in_grp'] . " " . $CurrentGroup . "</font><p>");
  ...
  // Now add the periods
  for($p=1; $p<=$periods; $p++)
    echo("<td><center><a href=viewsubjectgradedetails.php?period=$p&cid=" . $subject_array['cid'][1] . ">". $dtext['Period_marker']. $p . "</a></td>");
