...
//
  ...
  $sql_query = "SELECT result,type,mid,testdef.tdid FROM testresult LEFT JOIN testdef using (tdid) LEFT JOIN class USING (cid) LEFT JOIN period ON(period.id=testdef.period) where sid='$sid' AND period='$period' AND period.year=testdef.year ORDER BY last_update";
  $mysql_query = $sql_query;
  //echo $sql_query;
  $sql_result = mysql_query($mysql_query,$userlink);
  echo mysql_error($userlink);
  ...
  echo("<font size=+2><center>" . $dtext['gcard_4'] . " " . $student_array['firstname'][1] . " " . $student_array['lastname'][1] . " " . $dtext['4_per'] . " " .$period. "</font><p>");
