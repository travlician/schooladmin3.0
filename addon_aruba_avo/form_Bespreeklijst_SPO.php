		...
		foreach($students AS $student)
		{
		  ...
		  // Get the student absence in set
			...
			if($databasename=="ssclvs")
				$sres = SA_loadquery("SELECT SUM(IF(date >= '". $pdata[$repper]['sdate']. "' AND date <= '". $pdata[$repper]['edate']. "' AND (acid=7),1,0)) AS afw, SUM(IF(date >= '". $pdata[$repper]['sdate']. "' AND date <= '". $pdata[$repper]['edate']. "' AND acid=2,1,0)) AS late FROM absence LEFT JOIN absencereasons USING(aid) WHERE sid=". $student->get_id());
			else
				$sres = SA_loadquery("SELECT SUM(IF(date >= '". $pdata[$repper]['sdate']. "' AND date <= '". $pdata[$repper]['edate']. "' AND (acid=1 OR acid>3),1,0)) AS afw, SUM(IF(date >= '". $pdata[$repper]['sdate']. "' AND date <= '". $pdata[$repper]['edate']. "' AND acid=2,1,0)) AS late FROM absence LEFT JOIN absencereasons USING(aid) WHERE sid=". $student->get_id());
			if(isset($sres))
			{
