...
//
    ...
    //echo("Processing I&S results<BR>");
		...
		if(isset($students))
		{
			...
			foreach($students['sid'] AS $sid)
			{
				...
				//  echo(" No I&S result!");
				//echo("<BR>");
				// 20 mrt 2018: added query to copy PWS results from last year
				if($pfwmid > 0) // Only if we found and mid for this subject
				{
					$pfwqr = SA_loadquery("SELECT MAX(xstatus) AS pfwres FROM ahxdata WHERE sid=". $sid. " AND mid=". $pfwmid. " GROUP BY mid");		  
				} 
				if(isset($pfwqr['pfwres']))
				{
					$pfwres=$pfwqr['pfwres'][1];
					//echo(" Using PFW result: ". $pfwres. " for sid ". $sid);
					mysql_query("REPLACE INTO ahxdata (sid,mid,xstatus,year) VALUES(". $sid. ",". $pfwmid. ",'". $pfwres. "','". $curyear. "')", $userlink);
					echo(mysql_error($userlink));
				}
				//else
					//echo("<BR>No PWS result for ". $sid);
			}
		}
    //exit;
  }
 ...
 echo("</table>");
  ...
  echo("<FORM METHOD=POST ACTION='". $_SERVER['PHP_SELF']. "'><INPUT type=hidden name=getisresults value=1><input type=SUBMIT value='Ophalen I&S, PWS en LO resultaten vorige jaren'></FORM>");
