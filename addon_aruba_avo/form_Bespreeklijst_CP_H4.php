...
//
  ...
  $vakhead["Wiskunde"] = array("wiA","wiB","wiD","rek");
  $newan = array("ne","en","wi","asw","n&t");
  $ptvakken = array("i&s","lo","ec","m&o","ak","gs","ne","en","sp","pap","ckv","na","sk","bio","inf","wiA","wiB","wiD","rek");
  ...
  */
  ...
  if(isset($groups))
  {
    ...
    foreach($groups['gid'] AS $gix => $gid)
	{
	  ...
	  $sdquery .= " UNION SELECT type, fullname, shortname, GROUP_CONCAT(DISTINCT data) FROM class LEFT JOIN subject USING(mid) LEFT JOIN ". $teachercode. " USING(tid) LEFT JOIN sgroup USING(gid) WHERE active=1 AND (groupname LIKE 'H4%') AND groupname NOT LIKE 'CB%' GROUP BY mid";
