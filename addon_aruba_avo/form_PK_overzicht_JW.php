...
//
  ...
  $sqlquery = "CREATE TABLE IF NOT EXISTS `avo_pk_data` (
    ...
    `lastmodifiedat` TIMESTAMP(6) NOT NULL ON UPDATE CURRENT_TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  ...
  echo("<form method=post action=form_PK_overzicht_JW.php ID=groupsel NAME=groupsel>" . $dtext['Group_Cap'] . 
...
/*  echo("<form method=post action=form_PK_overzicht.php ID=subjsel NAME=subjsel>" . $dtext['Subject'] . 
  ...
  echo("<form method=post action=form_PK_overzicht_JW.php ID=persel NAME=persel>" . $dtext['Period'] . 
...
/*  echo("<tr><td><center>Leerling</td>");
  ...
  foreach($students AS $stobj)
  {
   if(isset($stobj))
   {
	...
	{
	  $totacnt = (isset($hdata[$sid][$aspix]["V"]) ? $hdata[$sid][$aspix]["V"] : 0) +
	             (isset($hdata[$sid][$aspix]["M"]) ? $hdata[$sid][$aspix]["M"] : 0) +
	             (isset($hdata[$sid][$aspix]["O"]) ? $hdata[$sid][$aspix]["O"] : 0);
	  ...
	  echo("<TD class=per". $CurrentPer. ">". (isset($hdata[$sid][$aspix]["O"]) ? ROUND($hdata[$sid][$aspix]["O"] * 100.0 / $totacnt,0). "%" : " "). "</td>");
	}
	echo("</tr>");
   }
  }
