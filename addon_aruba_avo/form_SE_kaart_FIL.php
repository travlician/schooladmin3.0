...
//  require_once("vakpakGPK.php");
  ...
  $altsn = array("NED"=>"ne","PAP"=>"pa","ENG"=>"en","WIS"=>"wi","SPA"=>"sp","SK"=>"nask 2","SCH"=>"nask 2","GES"=>"gs","NA"=>"nask 1","NASK2"=>"nask 2","NASK1"=>"nask 1","EC"=>"ecmo","EM & O"=>"ecmo","BI"=>"bio","EC/MO"=>"ecmo");
...
// foutmelding opvangen als er niet gekozen is voor een examenklas:
	...
	if(isset($pparts[1]))
	{
	  $extravakken = explode(",",strtolower($pparts[1]));
	}
	else
		unset($extravakken);
...
// 	Paragraaf SO uitslag:
	echo("<table class=OpmaakKaart><tr class=Dikte3LijnOnder><td class = breedte1>Examenonderdelen</td><td class = breedte2a>Vak</td>
			...
			<td class=breedte2 colspan=2>S.E. 3<BR><SPAN class=HerSubText>her</SPAN></td><td class=breedte2b class=Dikte3LijnTop>S.E. gem.</td>
	...
	if(substr($package,0,2) == "HX")
	  ...
	  echo("<td rowspan=5 class=DunneLijnOnderEnLinks class=SierTekst>");
	...
	// Hier komt het verplichte vak LO:
		...
		// 	Paragraaf Examenuitslag: SO-gem - CSE(+HER) - Einduitslag:
		echo("<td class=Tekstgrootte>
		<center>". (isset($averages[$subj2mid[$subjseq[$package][2]]][2]) ? $averages[$subj2mid[$subjseq[$package][2]]][2] : (isset($vrijst[$subj2mid[$subjseq[$package][2]]]) ? "&nbsp" : "-&#120;-")). "</center></td><td class=DunneLijnen>
		<center>". (isset($vrijst[$subj2mid[$subjseq[$package][2]]]) ? "&nbsp" : (isset($results[$subj2mid[$subjseq[$package][2]]]["Ex"]) ? $results[$subj2mid[$subjseq[$package][2]]]["Ex"] : "-&#120;-")). "</center></td><td class=LijnenWegHerBG>
		<center>". (isset($vrijst[$subj2mid[$subjseq[$package][2]]]) ? "&nbsp" : (isset($results[$subj2mid[$subjseq[$package][2]]]["Hex"]) ? $results[$subj2mid[$subjseq[$package][2]]]["Hex"] : "-&#120;-")). "</center></td><td class=Dikte3LijnRechts>
		<center>". (isset($vrijst[$subj2mid[$subjseq[$package][2]]]) ? "v(". $vrijst[$subj2mid[$subjseq[$package][2]]]. ")" : (isset($results[$subj2mid[$subjseq[$package][2]]]["Ex"]) || isset($results[$subj2mid[$subjseq[$package][2]]]["Hex"]) ? $averages[$subj2mid[$subjseq[$package][2]]][0] : "-&#120;-")). "</center></td>
	</tr>");
	...
	// Hier komt het verplichte vak Rekenen:
		...
		// 	Paragraaf Examenuitslag: SO-gem - CSE(+HER) - Einduitslag:
		echo("<td class=Tekstgrootte>
		<center>". (isset($averages[$subj2mid["rek"]][2]) ? $averages[$subj2mid["rek"]][2] : "-&#120;-"). "</center></td><td class=DunneLijnen>
		<center>&nbsp;</center></td><td class=LijnenWegHerBG>
		<center>&nbsp;</center></td><td class=Dikte3LijnRechts>
		<center>". (isset($vrijst[$subj2mid["rek"]]) ? "v(". $vrijst[$subj2mid["rek"]]. ")" : "&nbsp;"). "</center></td>
	</tr>");
	...
	$sogpass = (($sogminp < 2 || ($sogminp == 2 && $sogcomp >= 2)) && (isset($ckvres['ckvres']) && $ckvres['ckvres'][0] == 1) && ((isset($averages[$subj2mid[$subjseq[$package][2]]][2]) && $averages[$subj2mid[$subjseq[$package][2]]][2] > 5.4) || isset($vrijst[$subj2mid[$subjseq[$package][2]]])));
