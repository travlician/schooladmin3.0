...
//
  ...
  $vakdesc = array("kgl"=>"<B>Kennis van het Geestelijk Leven</B> <i>(Conocemento Spiritual y Religioso)</i>",
    ...
    foreach($groups['gid'] AS $gix => $gid)
	{
	  ...
	  if(isset($students))
	  {
		...
		while ($llnoffset < sizeof($students['sid']))
		{
		  ...
		  // Show frontpage
		  echo("<DIV class=frontpage><img src=schoollogo.png class=frontlogo width=50%><P class=schoolnamefront>Scol Practico pa Ofishi<BR>Locatie ". (substr($_SESSION['CurrentGroup'],1,1) == "C" ? "Santa Cruz" : "Savaneta"). "<P class=rapdblock><SPAN class=raplabel>Naam:</SPAN>");
		  ...
		  echo("<SPAN class=rapdata>". $mentorname. "</SPAN>");
			echo("<P class=frontfooter>Aña Escolar<BR>". $schoolyear. "</p>");
		  //echo("<BR><SPAN class=raplabel>Mentor:</SPAN>");
		  ...
		  echo("<TABLE class=cijferlijst>");
			echo("<TR><TD class=schooldata COLSPAN=2><SPAN class=schoolname>Scol Practico pa Ofishi</span><BR><SPAN class=schoolyear>- di ". (substr($_SESSION['CurrentGroup'],2,1) == "1" ? "Prome" : (substr($_SESSION['CurrentGroup'],2,1) == "2" ? "Dos" : "Tres")). " aña -</span></td>");
			echo("<td class=raphead rowspan=3>1<sup>e</sup><BR><BR>r<BR>a<BR>p<BR>o<BR>r<BR>t</td>");
			echo("<td class=raphead rowspan=3>2<sup>do</sup><BR><BR>r<BR>a<BR>p<BR>o<BR>r<BR>t</td>");
			echo("<td class=raphead rowspan=3>3<sup>er</sup><BR><BR>r<BR>a<BR>p<BR>o<BR>r<BR>t</td>");
			echo("<td class=raphead rowspan=3>Ultimo<BR><BR>r<BR>a<BR>p<BR>o<BR>r<BR>t</td>");
			echo("</tr><TR><TD class=headinfo colspan=2>Nomber: ". $students['firstname'][1+$llnoffset]. " ". $students['lastname'][1+$llnoffset]. "</td></tr>");
			echo("<TR><TD class=headinfo colspan=2>Klas: ". $_SESSION['CurrentGroup']. "</td></tr>");
		  ...
		  for($sx = 1; $sx <= $scnt; $sx++)
		  {
				...
				if($databasename=="ssclvs")
					$sres = SA_loadquery("SELECT SUM(IF(date >= '". $pdata[1]['sdate']. "' AND date <= '". $pdata[1]['edate']. "' AND (acid=7),1,0)) AS afw1, SUM(IF(date >= '". $pdata[2]['sdate']. "' AND date <= '". $pdata[2]['edate']. "' AND (acid=7),1,0)) AS afw2, SUM(IF(date >= '". $pdata[3]['sdate']. "' AND date <= '". $pdata[3]['edate']. "' AND (acid=7),1,0)) AS afw3, SUM(IF(date >= '". $pdata[1]['sdate']. "' AND date <= '". $pdata[1]['edate']. "' AND acid=2,1,0)) AS late1, SUM(IF(date >= '". $pdata[2]['sdate']. "' AND date <= '". $pdata[2]['edate']. "' AND acid=2,1,0)) AS late2, SUM(IF(date >= '". $pdata[3]['sdate']. "' AND date <= '". $pdata[3]['edate']. "' AND acid=2,1,0)) AS late3 FROM absence LEFT JOIN absencereasons USING(aid) WHERE sid=". $students['sid'][$llnoffset+$sx]);
				else
		  ...
		  {
				echo("<TR><TD class=vakcat ROWSPAN=". count($vakhead[$vk]). ">". $vk. "</td>");
		    $lastsubj = end($vakhead[$vk]);
				foreach($vakhead[$vk] AS $vkn)
				{
				 //if((substr($groups['groupname'][$gix],0,1) == 1 && $vkn != "ASWcb2") || 
				 //   (substr($groups['groupname'][$gix],0,1) == 2 && $vkn != "asw"))
				 {
					$lastmark = ($lastsubj == $vkn);
					echo("<TD class=subjectcol>". $subjdata[$vkn]['fullname']. "</TD>");
						for($sx = 1; $sx <= $scnt; $sx++)
						{
						echo("<TD class=resultcol>");
					if(isset($stres[$llnoffset+$sx][$vkn][1]))
						echo(colored($stres[$llnoffset+$sx][$vkn][1] > 0.9 ? number_format($stres[$llnoffset+$sx][$vkn][1],1,',','.') : $stres[$llnoffset+$sx][$vkn][1]));
					else
						echo("&nbsp;");
					echo("</TD>");
						echo("<TD class=resultcol>");
					if(isset($stres[$llnoffset+$sx][$vkn][2]) && $repper > 1)
						echo(colored($stres[$llnoffset+$sx][$vkn][2] > 0.9 ? number_format($stres[$llnoffset+$sx][$vkn][2],1,',','.') : $stres[$llnoffset+$sx][$vkn][2]));
					else
						echo("&nbsp;");
					echo("</TD>");
						echo("<TD class=resultcol>");
					if(isset($stres[$llnoffset+$sx][$vkn][3]) && $repper > 2)
						echo(colored($stres[$llnoffset+$sx][$vkn][3] > 0.9 ? number_format($stres[$llnoffset+$sx][$vkn][3],1,',','.') : $stres[$llnoffset+$sx][$vkn][3]));
					else
						echo("&nbsp;");
					echo("</TD>");
						echo("<TD class=resultcol>");
		  ...
		  echo("<TR><TD class=vakcat rowspan=2>Comportacion</td><TD class=subjectcol>Comportacion pa cu estudio</TD>
			<TD class=resultcol>". (isset($behave[$students['sid'][$llnoffset+1]]['Gedrag'][1]) ? $behave[$students['sid'][$llnoffset+1]]['Gedrag'][1] : '&nbsp;'). "</TD>
			<TD class=resultcol>". (isset($behave[$students['sid'][$llnoffset+1]]['Gedrag'][2]) ? $behave[$students['sid'][$llnoffset+1]]['Gedrag'][2] : '&nbsp;'). "</TD>
			<TD class=resultcol>". (isset($behave[$students['sid'][$llnoffset+1]]['Gedrag'][3]) ? $behave[$students['sid'][$llnoffset+1]]['Gedrag'][3] : '&nbsp;'). "</TD>
			<TD class=resultcol>&nbsp;</TD></TR>");		  
		  echo("<TR><TD class=subjectcol>Actitud pa cu estudio</TD>
			<TD class=resultcol>". (isset($behave[$students['sid'][$llnoffset+1]]['Houding'][1]) ? $behave[$students['sid'][$llnoffset+1]]['Houding'][1] : '&nbsp;'). "</TD>
			<TD class=resultcol>". (isset($behave[$students['sid'][$llnoffset+1]]['Houding'][2]) ? $behave[$students['sid'][$llnoffset+1]]['Houding'][2] : '&nbsp;'). "</TD>
			<TD class=resultcol>". (isset($behave[$students['sid'][$llnoffset+1]]['Houding'][3]) ? $behave[$students['sid'][$llnoffset+1]]['Houding'][3] : '&nbsp;'). "</TD>
			<TD class=resultcol>&nbsp;</TD></TR>");		  
		  //echo("<TR><TD class=thickLR><CENTER><B>Vlijt</B></CENTER></TD><TD class=resultcol>&nbsp;</TD><TD class=resultcol>&nbsp;</TD><TD class=resultcolE>&nbsp;</TD></TR>");	
			echo("<TR><TD class=spacerrow colspan=6>&nbsp;</td></tr>");
		  echo("<TR><TD class=vakcat rowspan=2>Ausencia</td><TD class=subjectcol>Total di ausencia</TD><TD class=resultcol>". (isset($stabs[$llnoffset+1][1]) ? $stabs[$llnoffset+1][1] : "&nbsp;"). "</TD><TD class=resultcol>". (isset($stabs[$llnoffset+1][2]) ?  $stabs[$llnoffset+1][2] : "&nbsp;"). "</TD><TD class=resultcol>". (isset($stabs[$llnoffset+1][3]) ? $stabs[$llnoffset+1][3] : "&nbsp;"). "</TD><TD class=resultcol>". ($repper > 2 ? $stabs[$llnoffset+1][0] : "&nbsp"). "</TD></TR>");		  
		  echo("<TR><TD class=subjectcol>Yegamento lat</TD><TD class=resultcol>". (isset($stlate[$llnoffset+1][1]) ? $stlate[$llnoffset+1][1] : "&nbsp;"). "</TD><TD class=resultcol>". (isset($stlate[$llnoffset+1][2]) ? $stlate[$llnoffset+1][2] : "&nbsp;"). "</TD><TD class=resultcol>". (isset($stlate[$llnoffset+1][3]) ? $stlate[$llnoffset+1][3] : "&nbsp;"). "</TD><TD class=resultcol>". ($repper > 2 ? $stlate[$llnoffset+1][0] : "&nbsp"). "</TD></TR>");
...
*/
		  ...
		  echo("<TABLE class=opmblock>");
			echo("<TR><TD class=opmyear>Aña Escolar<SPAN class=opmyeart>". $schoolyear. "</span></td>
			<td class=opmlogo rowspan=2><img src=schoollogo.png width=40%><BR>Scol Practico pa Ofishi<BR>Locatie ". (substr($_SESSION['CurrentGroup'],1,1) == "C" ? "Santa Cruz" : "Savaneta"). "</td></tr>");
			echo("<TR><TD class=opmmentorname>Mentor: ". $mentorname. "</td></tr>");
			echo("<TR class=opmsep><TD>di prome raport</TD><TD>Fecha:</TD></TR>");
		  echo("<TR><TD colspan=2>Rapport:<BR>Remarka:<BR>");
			if(isset($remarks[$students['sid'][$llnoffset+1]][1]))
				echo($remarks[$students['sid'][$llnoffset+1]][1]);
			else
				echo("<BR>&nbsp;");
			echo("<BR>Firma:</TD></TR>");
			echo("<TR><TD class=opmfirm>Mentor</td><TD class=opmfirm>Mayor / Tutela</td></tr>");
			echo("<TR class=opmsep><TD>di dos raport</TD><TD>Fecha:</TD></TR>");
		  echo("<TR><TD colspan=2>Rapport:<BR>Remarka:<BR>");
			if(isset($remarks[$students['sid'][$llnoffset+1]][2]))
				echo($remarks[$students['sid'][$llnoffset+1]][2]);
			else
				echo("<BR>&nbsp;");
			echo("<BR>Firma:</TD></TR>");
			echo("<TR><TD class=opmfirm>Mentor</td><TD class=opmfirm>Mayor / Tutela</td></tr>");
			echo("<TR class=opmsep><TD>ultimo raport</TD><TD>Fecha:</TD></TR>");
		  echo("<TR><TD colspan=2>Rapport:<BR>Remarka:<BR>");
			if(isset($remarks[$students['sid'][$llnoffset+1]][3]))
				echo($remarks[$students['sid'][$llnoffset+1]][3]);
			else
				echo("<BR>&nbsp;");
			echo("<BR>No ta pasa / Ta pasa pa:<BR><BR>Firma:</TD></TR>");
			echo("<TR><TD class=opmfirm>Mentor</td><TD class=opmfirm>Mayor / Tutela</td></tr>");
		  echo("</TABLE>");
