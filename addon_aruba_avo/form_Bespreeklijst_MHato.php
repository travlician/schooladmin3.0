...
//  $vakhead["Kunst & Cultuur"] = array("Dr","Mu","Ckv");
  $vakhead["Kunst & Cultuur"] = array("Bv","Mu","Ckv");
    ...
    foreach($groups['gid'] AS $gix => $gid)
	{
	  ...
	  if(isset($students))
	  {
		...
		while ($llnoffset < sizeof($students['sid']))
		{
		  ...
		  echo("<TR><TD COLSPAN=2>NEWAN onvold.</TD>");
		  for($sx = 1; $sx <= $scnt; $sx++)
		  {
		    for($p=0;$p<4;$p++)
			{
			  $pi = ($p + 1) % 4;
			  echo("<TD>");
			  if(isset($stcalno[$sx][$pi]) && $stcalno[$sx][$pi] > 0)
			    echo($stcalno[$sx][$pi]);
			  else
			    echo("&nbsp");
			  echo("</TD>");
		    ...
		    for($p=0;$p<4;$p++)
			{
			  ...
			  if($stcalp[$sx][$pi] >= 88 && $stcalo[$sx][$pi] == 0 && $stcaln[$sx][$pi] >= 40)
			    echo("HV");
			          ...
			          ($stcalp[$sx][$pi] == 71 && $stcalo[$sx][$pi] == 1 && $stcalc[$sx][$pi] == 1))
			    echo("BV");
			  else if($stcalp[$sx][$pi] >= 66 && $stcalo[$sx][$pi] <= 4 && $stcalc[$sx][$pi] <= 6)
			    echo("BE");
			  else if($stcalp[$sx][$pi] < 55 || $stcalno[$sx][$pi] == 5)
			    echo("EPB");
			  else if($stcalp[$sx][$pi] < 66 &&  $stcalp[$sx][$pi] >= 55)
			    echo("DB");
			  else
			    echo("?");
			  echo("</TD>");
		  ...
		  echo("</TABLE>");
		  echo("<P class=footer>Advies codes: HV=HAVO, BV=Bevorderen, BE=Bespreekgeval, DB=Doubleren, EPB=EPB, ?=Niet vermeld in afspraken DO</P>");
