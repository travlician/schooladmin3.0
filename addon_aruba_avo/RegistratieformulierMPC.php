...
//
//
// Dit inschrijfformulier is gemaakt voor de Mon Plaisir College
  session_start();
 ...
 // $login_qualify = 'ACT';
  ...
  $sqlquery = "CREATE TABLE IF NOT EXISTS `inschrijvingMPC` (
  ...
  echo(mysql_error());
	...
	// Add the new columns
	mysql_query("ALTER TABLE inschrijvingMPC ADD COLUMN proof TEXT DEFAULT NULL AFTER remarks", $userlink);
	mysql_query("ALTER TABLE inschrijvingMPC ADD COLUMN schooltype TEXT DEFAULT NULL AFTER year", $userlink);
	mysql_query("ALTER TABLE inschrijvingMPC ADD COLUMN fbaptised TEXT DEFAULT NULL AFTER religionfather", $userlink);
	mysql_query("ALTER TABLE inschrijvingMPC ADD COLUMN mbaptised TEXT DEFAULT NULL AFTER religionmother", $userlink);
	mysql_query("ALTER TABLE inschrijvingMPC ADD COLUMN brusonschool TEXT DEFAULT NULL AFTER remarks", $userlink);
	mysql_query("ALTER TABLE inschrijvingMPC ADD COLUMN workSPCOA TEXT DEFAULT NULL AFTER brusonschool", $userlink);
	mysql_query("ALTER TABLE inschrijvingMPC ADD COLUMN exMBstudent TEXT DEFAULT NULL AFTER workSPCOA", $userlink);
  ...
  { // Convert registration code to rid (record in registation table)
    $rid=base64_decode(urldecode($_GET['regcode']));
		$typeyrqr = inputclassbase::load_query("SELECT schooltype,year FROM inschrijvingMPC WHERE rid=". $rid);
	...
	{ // existing record, get type and year to set
		$typeyrqr = inputclassbase::load_query("SELECT schooltype,year FROM inschrijvingMPC WHERE rid=". $_POST['rid']);
		if(isset($typeyrqr['schooltype'][0]) && $typeyrqr['schooltype'][0] != "")
			$_GET[$typeyrqr['schooltype'][0]] = $typeyrqr['year'][0];		
	}
  ...
  // Get the year
	...
	echo("<P class=topheading>AANMELDINGSFORMULIER<BR>Mon Plaisir College<BR>". $schoolyear. "</P>");
  ...
  else if(isset($_POST['userid']))
  {
	...
	{ // Check login for this student
	  ...
	  { // student ID is retrieved from database based on login data, now check if a record already exists or create one
	    $ridqr = inputclassbase::load_query("SELECT rid,paid FROM inschrijvingMPC WHERE sid=". $logindata['sid'][0]. " AND year='". $schoolyear. "'");
			if(isset($ridqr['rid']))
			...
			{ // Known student but no registration record filed yet, so create a new record with base info
				...
				$insrecq = "INSERT INTO inschrijvingMPC (year,sid,firstname,lastname,gebdag,gebmaand,gebjaar,lvsid,lvspassword)";
				...
				foreach($reg2stud AS $ikey => $dkey)
				{
					...
					{
						mysql_query("UPDATE inschrijvingMPC SET ". $ikey. "='". $ddata['data'][0]. "' WHERE rid=". $rid,$userlink);
						echo(mysql_error($userlink));
					}
				}
			}
  ...
  if(isset($_POST['rid']))
		echo("<a href=form_Inschrijven_MPC.php>Terug naar het zoekscherm</a>");
  ...
  $lnlnfld = new inputclass_textfield("slname",40,NULL,"lastname","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  ...
  $lnfnfld = new inputclass_textfield("sfname",40,NULL,"firstname","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfnfld->echo_html();
  echo("<BR><LABEL>Geslacht / Sexo:</LABEL> ");
  $lnfld = new inputclass_listfield("sgender","SELECT '' AS id,'' AS tekst UNION SELECT 'M','Man' UNION SELECT 'V','Vrouw'",NULL,"geslacht","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Geboortedatum / Fecha di nacemento:</LABEL> ");
  $bdayq = "SELECT '' AS id, '' AS tekst";
  for($d=1;$d<=31;$d++)
    $bdayq .= " UNION SELECT '". ($d < 10 ? "0" : ""). $d. "','". ($d < 10 ? "0" : ""). $d. "'";
  $lnfld = new inputclass_listfield("sbday",$bdayq,NULL,"gebdag","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo(" ");
  $bmonq = "SELECT '' AS id, '' AS tekst";
  for($d=1;$d<=12;$d++)
    $bmonq .= " UNION SELECT '". $montxt[$d]. "','". $montxt[$d]. "'";
  $lnfld = new inputclass_listfield("sbmon",$bmonq,NULL,"gebmaand","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo(" ");
  $byrq = "SELECT '' AS id, '' AS tekst";
	for($d=11;$d<=14;$d++)
		$byrq .= " UNION SELECT '". (date("Y")-$d). "','". (date("Y")-$d). "'";
  $lnfld = new inputclass_listfield("sbyr",$byrq,NULL,"gebjaar","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Censo ID:</LABEL> ");
  $lnfld = new inputclass_textfield("scensoid",8,NULL,"censoid","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Protestant Christelijk:</LABEL> ");
  $lnfld = new inputclass_listfield("sprotestant","SELECT '' AS id, '' AS tekst UNION SELECT 'n' AS id, 'Nee/no' AS tekst UNION SELECT 'j','Ja/si'",NULL,"protestant","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Religie / Religion:</LABEL> ");
  $lnfld = new inputclass_textfield("sreligion",20,NULL,"religionstudent","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>A.Z.V. Relatienummer / Number di A.Z.V.:</LABEL> ");
  $lnfld = new inputclass_textfield("sazv",40,NULL,"medicalinsurancenumber","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Geboorteland / Pais di nacemento:</LABEL> ");
  $lnfld = new inputclass_listfield("sbplace","SELECT '' AS id,'' AS tekst UNION SELECT id,tekst FROM arubacom.c_country ORDER BY tekst",NULL,"gebland","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Nationaliteit / Nacionalidad:</LABEL> ");
  $lnfld = new inputclass_listfield("snationality","SELECT '' AS id,'' AS tekst UNION SELECT id,tekst FROM arubacom.c_nationality ORDER BY tekst",NULL,"nationality","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Meest gesproken taal thuis / Idioma mas papia na cas:</LABEL> ");
  $lnfld = new inputclass_listfield("shlanguage","SELECT '' AS id,'' AS tekst UNION SELECT id,tekst FROM arubacom.c_language ORDER BY tekst",NULL,"homelanguage","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Andere talen di thuis gesproken worden / <BR>Otro idiomanan papia na cas:</LABEL> ");
  $lnfld = new inputclass_textfield("sotherhomelangs",40,NULL,"otherhomelanguages","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Adres:</LABEL> ");
  $lnfld = new inputclass_textfield("sadres",40,NULL,"adres","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>District / Districto:</LABEL> ");
  $lnfld = new inputclass_listfield("sdistrict","SELECT '' AS id,'' AS tekst UNION SELECT id,tekst FROM arubacom.c_district ORDER BY tekst",NULL,"district","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Op Aruba woonachtig sinds / Biba na Aruba desde:</LABEL> ");
  $lnfld = new inputclass_textfield("sonarubasince",40,NULL,"onarubasince","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("</fieldset>");
  ...
  $lnfld = new inputclass_listfield("sresponsable",$liveatsel,NULL,"responsableperson","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
	echo(" Anders / Otro: ");
  $lnfld = new inputclass_textfield("srespother",40,NULL,"responsablealternative","inschrijvingMPC",$rid,"rid","visibility: hidden;","inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Telefoon bij noodgeval / Telefon di emergencia:</LABEL> ");
  $lnfld = new inputclass_textfield("emergencyphone",40,NULL,"emergencyphone","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Leerling is woonachtig bij / Alumno ta biba cerca:</LABEL> ");
  $lnfld = new inputclass_listfield("sliveat",$liveatsel,NULL,"liveat","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
	echo(" Anders / Otro: ");
  $lnfld = new inputclass_textfield("sliveatother",40,NULL,"liveatother","inschrijvingMPC",$rid,"rid","visibility: hidden;","inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>E-mail adres:</LABEL> ");
  $lnfld = new inputclass_textfield("semail",40,NULL,"email","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  ...
  $lnfld = new inputclass_textfield("mfam",40,NULL,"lastnamemother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Voorna(a)m(en) (voluit) / Nomber(nan) completo:</LABEL> ");
  $lnfld = new inputclass_textfield("mfname",40,NULL,"firstnamesmother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Adres:</LABEL> ");
  $lnfld = new inputclass_textfield("maddress",40,NULL,"addressmother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Nationaliteit / Nacionalidad:</LABEL> ");
  $lnfld = new inputclass_listfield("mnationality","SELECT '' AS id,'' AS tekst UNION SELECT id,tekst FROM arubacom.c_nationality ORDER BY tekst",NULL,"nationalitymother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Geboortedatum / Fecha di nacemento:</LABEL> ");
  $lnfld = new inputclass_textfield("mbirthdate",20,NULL,"birthdatemother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Religie / Religion:</LABEL> ");
  $lnfld = new inputclass_listfield("mreligion","SELECT '' AS id, '' AS tekst UNION SELECT * FROM arubacom.c_religion",NULL,"religionmother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Telefoon thuis / Telefon na cas:</LABEL> ");
  $lnfld = new inputclass_textfield("mhomephone",40,NULL,"phonehomemother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Mobiel / Celular:</LABEL> ");
  $lnfld = new inputclass_textfield("mmobilephone",40,NULL,"phonemobilemother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>E-mail:</LABEL> ");
  $lnfld = new inputclass_textfield("memail",40,NULL,"emailmother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Beroep / Ocupacion:</LABEL> ");
  $lnfld = new inputclass_textfield("mprofession",40,NULL,"professionmother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Werkt bij / Ta traha na:</LABEL> ");
  $lnfld = new inputclass_textfield("memployer",40,NULL,"employermother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Telefoon op het werk / Telefon na trabou:</LABEL> ");
  $lnfld = new inputclass_textfield("mworkphone",40,NULL,"phoneworkmother","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("</fieldset>");
  ...
  $lnfld = new inputclass_textfield("ffam",40,NULL,"lastnamefather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Voorna(a)m(en) (voluit) / Nomber(nan) completo:</LABEL> ");
  $lnfld = new inputclass_textfield("ffname",40,NULL,"firstnamesfather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Adres:</LABEL> ");
  $lnfld = new inputclass_textfield("faddress",40,NULL,"addressfather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Nationaliteit / Nacionalidad:</LABEL> ");
  $lnfld = new inputclass_listfield("fnationality","SELECT '' AS id,'' AS tekst UNION SELECT id,tekst FROM arubacom.c_nationality ORDER BY tekst",NULL,"nationalityfather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Geboortedatum / Fecha di nacemento:</LABEL> ");
  $lnfld = new inputclass_textfield("fbirthdate",20,NULL,"birthdatefather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Religie / Religion:</LABEL> ");
  $lnfld = new inputclass_listfield("freligion","SELECT '' AS id, '' AS tekst UNION SELECT * FROM arubacom.c_religion",NULL,"religionfather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Telefoon thuis / Telefon na cas:</LABEL> ");
  $lnfld = new inputclass_textfield("fhomephone",40,NULL,"phonehomefather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Mobiel / Celular:</LABEL> ");
  $lnfld = new inputclass_textfield("fmobilephone",40,NULL,"phonemobilefather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>E-mail:</LABEL> ");
  $lnfld = new inputclass_textfield("femail",40,NULL,"emailfather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Beroep / Ocupacion:</LABEL> ");
  $lnfld = new inputclass_textfield("fprofession",40,NULL,"professionfather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Werkt bij / Ta traha na:</LABEL> ");
  $lnfld = new inputclass_textfield("femployer",40,NULL,"employerfather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Telefoon op het werk / Telefon na trabou:</LABEL> ");
  $lnfld = new inputclass_textfield("fworkphone",40,NULL,"phoneworkfather","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("</fieldset>");
  ...
  $lnfld = new inputclass_listfield("famcivstate","SELECT '' AS id, '' AS tekst UNION SELECT * FROM arubacom.c_civilstate",NULL,"civilstateparents","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Huisarts / Docter di cas:</LABEL> ");
  $lnfld = new inputclass_textfield("fammedic",40,NULL,"familydoctor","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Kind woont samen met / E Mucha ta biba hunto cu:</LABEL> ");
  $lnfld = new inputclass_textfield("famconst",40,NULL,"familyconstellation","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  /*echo("<BR><LABEL>Botica:</LABEL> ");
  $lnfld = new inputclass_textfield("famfharmacy",40,NULL,"pharmacy","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html(); */
  echo("</fieldset>");
  ...
  $lnfld = new inputclass_textfield("speuter",40,NULL,"peuterschool","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Kleuterschool / Scol preparatorio</LABEL> ");
  $lnfld = new inputclass_textfield("skleuter",40,NULL,"kleuterschool","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Basisschool / Scol basico:</LABEL> ");
  $lnfld = new inputclass_listfield("sbaseschool","SELECT '' AS id, '' AS tekst UNION SELECT * FROM arubacom.c_BOschool",NULL,"primaryschool","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
	echo(" Anders / Otro: ");
  $lnfld = new inputclass_textfield("sotherbase",40,NULL,"primaryother","inschrijvingMPC",$rid,"rid","visibility: hidden;","inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Gedoubleerd op basisschool klas / Keda sinta den klas:</LABEL> ");
  $lnfld = new inputclass_textfield("sdoubled",8,NULL,"doublingclasses","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("</fieldset>");
  ...
  $lnfld = new inputclass_textfield("b1name",40,NULL,"brosname1","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>School broer of zus (1) / Scol di e ruman (1):</LABEL> ");
  $lnfld = new inputclass_textfield("b1school",40,NULL,"brosschool1","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Klas:</LABEL> ");
  $lnfld = new inputclass_textfield("b1class",8,NULL,"brosclass1","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Naam broer of zus (2) / Nomber di ruman (2):</LABEL> ");
  $lnfld = new inputclass_textfield("b2name",40,NULL,"brosname2","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>School broer of zus (2) / Scol di e ruman (2):</LABEL> ");
  $lnfld = new inputclass_textfield("b2school",40,NULL,"brosschool2","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Klas:</LABEL> ");
  $lnfld = new inputclass_textfield("b2class",8,NULL,"brosclass2","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Naam broer of zus (3) / Nomber di ruman (3):</LABEL> ");
  $lnfld = new inputclass_textfield("b3name",40,NULL,"brosname3","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>School broer of zus (3) / Scol di e ruman (3):</LABEL> ");
  $lnfld = new inputclass_textfield("b3school",40,NULL,"brosschool3","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Klas:</LABEL> ");
  $lnfld = new inputclass_textfield("b3class",8,NULL,"brosclass3","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Naam broer of zus (4) / Nomber di ruman (4):</LABEL> ");
  $lnfld = new inputclass_textfield("b4name",40,NULL,"brosname4","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>School broer of zus (4) / Scol di e ruman (4):</LABEL> ");
  $lnfld = new inputclass_textfield("b4school",40,NULL,"brosschool4","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Klas:</LABEL> ");
  $lnfld = new inputclass_textfield("b4class",8,NULL,"brosclass4","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("</fieldset>");
  echo("<FIELDSET style='background-color:#FFBBBB'><LEGEND>Ander informatie / Otro informacion</legend>");
	if(!isset($_GET['Herinschrijving']))
	{
		...
		$lnfld = new inputclass_textfield("motive",100,NULL,"motivation","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
		$lnfld->echo_html();
	}
  echo("<BR><LABEL>Is er een medisch probleem van belang voor de school / Tin problema medico cu e scol mester sa:</LABEL> ");
  $lnfld = new inputclass_listfield("smedicalyn","SELECT '' AS id, '' AS tekst UNION SELECT 'j', 'Ja / Si' UNION SELECT 'n','Nee / No'",NULL,"medicalprobsyn","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  ...
  $lnfld = new inputclass_textfield("smedicalprobs",100,NULL,"medicalprobs","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
	echo("</DIV>");

  echo("<LABEL>Is er een ontwikkelingsprobleem van belang voor de school / Tin problema den desaroyo cu e scol mester sa:</LABEL> ");
  $lnfld = new inputclass_listfield("sdevprobyn","SELECT '' AS id, '' AS tekst UNION SELECT 'j', 'Ja / Si' UNION SELECT 'n','Nee / No'",NULL,"developprobsyn","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  ...
  $lnfld = new inputclass_textfield("sdevprobs",100,NULL,"developprobs","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();

  echo("<LABEL>Is het kind hiervoor getest / A test e mucha pa cu esaki:</LABEL> ");
  $lnfld = new inputclass_listfield("stestedyn","SELECT '' AS id, '' AS tekst UNION SELECT 'j', 'Ja / Si' UNION SELECT 'n','Nee / No'",NULL,"testedyn","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  ...
  $lnfld = new inputclass_textfield("stestedwhen",10,NULL,"testedwhen","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
  echo("<BR><LABEL>Door wie is het kind getest / Ken a test e mucha:</LABEL> ");
  $lnfld = new inputclass_textfield("stestedby",40,NULL,"testedby","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
	echo("</DIV>");

  echo("<LABEL>Wordt het kind (nog) begeleid / E mucha (ainda) ta wordo guia:</LABEL> ");
  $lnfld = new inputclass_listfield("sguidedyn","SELECT '' AS id, '' AS tekst UNION SELECT 'j', 'Ja / Si' UNION SELECT 'n','Nee / No'",NULL,"guidedyn","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
	echo("<DIV ID=guidedetails style='visibility:hidden'>");
  echo("<LABEL>Door wie wordt het kind begeleid / Ken ta guia e mucha:</LABEL> ");
  $lnfld = new inputclass_textfield("sguidedby",10,NULL,"guidedby","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
	echo("</DIV>");
  ...
  $lnfld = new inputclass_listfield("sallowtests","SELECT '' AS id, '' AS tekst UNION SELECT 'j', 'Ja / Si' UNION SELECT 'n','Nee / No'",NULL,"allowtests","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
  $lnfld->echo_html();
...
<?
  ...
  if(isset($_POST['rid']))
  {
    echo("<FIELDSET style='background-color: #FFFFFF'><LEGEND>Inschrijving Mon Plaisir College</LEGEND>");

		echo("<BR><LABEL>Leerling kan naar MAVO / Alumno por bay MAVO:</LABEL> ");
		$lnfld = new inputclass_listfield("smavo","SELECT '' AS id,'' AS tekst UNION SELECT 'ja' AS id, 'Ja/Si' AS tekst UNION SELECT 'nee' AS id, 'Nee/No' AS tekst",NULL,"MAVOniveau","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
		$lnfld->echo_html();

		echo("<BR><LABEL>Leerling Protestants gedoopt / Alumno bautisa protestant:</LABEL> ");
		$lnfld = new inputclass_listfield("sbaptised","SELECT '' AS id,'' AS tekst UNION SELECT 'ja' AS id, 'Ja/Si' AS tekst UNION SELECT 'nee' AS id, 'Nee/No' AS tekst",NULL,"baptised","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
		$lnfld->echo_html();

		echo("<BR><LABEL>Vader Protestants gedoopt / Tata bautisa protestant:</LABEL> ");
		$lnfld = new inputclass_listfield("fbaptised","SELECT '' AS id,'' AS tekst UNION SELECT 'ja' AS id, 'Ja/Si' AS tekst UNION SELECT 'nee' AS id, 'Nee/No' AS tekst",NULL,"fbaptised","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
		$lnfld->echo_html();

		echo("<BR><LABEL>Moeder Protestants gedoopt / Mama bautisa protestant:</LABEL> ");
		$lnfld = new inputclass_listfield("mbaptised","SELECT '' AS id,'' AS tekst UNION SELECT 'ja' AS id, 'Ja/Si' AS tekst UNION SELECT 'nee' AS id, 'Nee/No' AS tekst",NULL,"mbaptised","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
		$lnfld->echo_html();

		echo("<BR><LABEL>Broer of zus op Mon Plaisirschool / Ruman na Mon Plaisirschool:</LABEL> ");
		$lnfld = new inputclass_listfield("brusonschool","SELECT '' AS id,'' AS tekst UNION SELECT 'ja' AS id, 'Ja/Si' AS tekst UNION SELECT 'nee' AS id, 'Nee/No' AS tekst",NULL,"brusonschool","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
		$lnfld->echo_html();

		echo("<BR><LABEL>Ouder werkt bij SPCOA / Mayor ta traha na SPCOA:</LABEL> ");
		$lnfld = new inputclass_listfield("workSPCOA","SELECT '' AS id,'' AS tekst UNION SELECT 'ja' AS id, 'Ja/Si' AS tekst UNION SELECT 'nee' AS id, 'Nee/No' AS tekst",NULL,"workSPCOA","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
		$lnfld->echo_html();

		echo("<BR><LABEL>Ouder(s) oud-leerling MPC:</LABEL> ");
		$lnfld = new inputclass_listfield("exMBstudent","SELECT '' AS id,'' AS tekst UNION SELECT 'ja' AS id, 'Ja/Si' AS tekst UNION SELECT 'nee' AS id, 'Nee/No' AS tekst",NULL,"exMBstudent","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
		$lnfld->echo_html();

    $sidfld = new inputclass_checkbox("sidrq",0,NULL,"sid","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
    if($sidfld->__toString() == '' || $sidfld->__toString() == 0 || 1==1)
		{  
			...
			if(isset($matchersqr['sid']))
			{
				...
				$mfld = new inputclass_listfield("smatch",$smatchqr,NULL,"sid","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
				$mfld->echo_html();
			}
			// Allow setting new ID and password
			echo("<BR><LABEL>Identificatienummer student:</LABEL> ");
			$lnfld = new inputclass_textfield("ssidnummer",10,NULL,"lvsid","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
			$lnfld->echo_html();
			echo(" (automatisch toegekend: ". (substr(date("Y"),2). str_pad($rid,4,"0",STR_PAD_LEFT)). ")");
			echo("<BR><LABEL>Wachtwoord student:</LABEL> ");
			$lnfld = new inputclass_textfield("sspasw",10,NULL,"lvspassword","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
			$lnfld->echo_html();
    ...
    $lnfld = new inputclass_checkbox("bewijs",0,NULL,"proof","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
    $lnfld->echo_html();
    echo("<BR><LABEL>Geaccepteerd:</LABEL> ");
    $lnfld = new inputclass_checkbox("geaccepteerd",0,NULL,"accepted","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
    $lnfld->echo_html();
    echo("<BR><LABEL>Betaald:</LABEL> ");
    $lnfld = new inputclass_checkbox("betaald",0,NULL,"paid","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
    $lnfld->echo_html();
    echo("<BR><LABEL>Opmerkingen:</LABEL> ");
    $lnfld = new inputclass_textarea("opmerkingen","60,*",NULL,"remarks","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
    $lnfld->echo_html();
    echo("<BR><LABEL>Plaatsen in leerjaar / lokatie:</LABEL> ");
    $lnfld = new inputclass_listfield("siljaar","SELECT '' AS id, '' AS tekst UNION SELECT gid,groupname FROM sgroup ORDER BY tekst",NULL,"registeredinclass","inschrijvingMPC",$rid,"rid",NULL,"inschrijfMPChandler.php");
    $lnfld->echo_html();
  ...
  else
  {
    ...
    echo("<BR><INPUT TYPE=SUBMIT VALUE='AFDRUKKEN/PRINT' onClick='window.print()'>U dient dit formulier, volledig ingevuld, af te drukken en mee te brengen naar school samen met de andere vereiste documenten / Mester print e formulier aki, completamente yena, y hibe scol hunto cu e otro documentnan exigi.");
		echo("<BR>De vereiste documenten zijn / E otro documentonan exigi ta :<BR>");
		echo("<OL><LI>Rapporten 5de en 6de klas basisschool / Raportnan di klas 5 y 6 di e scol basico</LI>");
		echo("<LI>Censo bewijs van inschrijving van Afl. 5.00/ Prueba di registracion di Censo di 5 florin</LI>");
		echo("<LI>Dit registratie formulier / E formulario di registracion aki</LI>");
		echo("<LI>Indien protestant, de doopbewijzen / Prueba di bautisacion si ta Protestant</LI></OL>");
    echo("<BR><INPUT TYPE=SUBMIT VALUE='VERSTUREN/MANDA' onClick='location.href=\"RegistratieformulierMPC.php?byebye=1\"'>");
  }
...
function send_xmlcb(fieldid,fieldobj)
{
  ...
  if (!myConn[fieldid]) alert("XMLHTTP not available. Try a newer/better browser.");
		if(fieldobj.checked == false)
			...
			cbstat = 1;
  myConn[fieldid].connect("inschrijfMPChandler.php", "POST", "fieldid="+fieldid+"&"+fieldobj.name+"="+cbstat, xmlconnDone);
}
</script>
<?
  // close the page
