...
//
  ...
  // We need to sort on exnr, so set that but store it for later recovery, only if month is not august or september!
//	if(date("m") != 8 && date("m") != 9)
	if(7==7) // 27 sep 2017, list must always be sorted acc to exnr
	{
		$orgsort=$_SESSION['ssortertable'];
		$_SESSION['ssortertable']="s_exnr";
	}
	else
...
// Clear existing exam numbers if in month august or september
	...
	{ // Autogen exam numbers
	  // First delete any exiting exam number
		// Deleted because manual requested 20 sep 2017
...
// 	Tabel met de gewenste gegevens van de leerling uit de betreffende klas:
		...
		foreach ($LLlijst AS $student)
		{
			if($student != NULL)
		 {
			...
			// Deleted becasue manual requested 20 sep 2017
			/* if(date('m') == 8 || date('m') == 9)
			{ // Autogen exam numbers
