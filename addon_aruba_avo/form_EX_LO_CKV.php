...
//
  ...
  if(isset($loresqr['sid']))
  {
	...
	// 23 april 2018:
	// If an LO result is presnt in EX1-5 invoer we need to use that
	$loforqr = SA_loadquery("SELECT sid,xstatus FROM ahxdata LEFT JOIN subject USING(mid) WHERE year='" . $schoolyear. "' AND shortname='lo' AND xstatus IS NOT NULL");
	if(isset($loforqr['sid']))
		foreach($loforqr['sid'] AS $six => $asid)
			$lores[$asid] = round($loforqr['xstatus'][$six]);
