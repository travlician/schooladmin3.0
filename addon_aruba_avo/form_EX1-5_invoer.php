...
//
  ...
  $altsubjects = array("NAT4"=>6, "GES4"=>11, "AK4"=>10, "EC4"=>9, "BIO4"=>8, "SCH4"=>7, "WIS4"=>5, "PAP4"=>4, "SPA4"=>3, "ENG4"=>2, "NED4"=>1,
					   ...
					   "NA"=>6, "EC"=>9, "EM & O"=>9, "CKV"=>12, "Ckv"=>12, "NED"=>1, "ENG"=>2, "SPA"=>3, "PAP"=>4, "WIS"=>5, "NASK1" => 6,"NS2"=>7, "NASK2" => 7, "BIO"=>8, "GES"=>11, "CKV"=>12, "bio"=>8, "CKV alg"=>12, "ecmo"=>9,"Nask 1"=>6,"Nask 2"=>7,"LO"=>13,"Rek"=>14);
  ...
  { // Need to extract results from previous year!
    $years = SA_loadquery("SELECT DISTINCT year FROM gradestore ORDER BY year DESC");
		...
		{ // The previous year exists (very important...)
			...
			$pyrq .= "WHERE active=1 AND year='". $prevyear. "' AND groupname LIKE '4%' GROUP BY sid,mid ORDER BY sid,mid,period";
			$pygrades = SA_loadquery($pyrq);
			foreach($pygrades['sid'] AS $pyrix => $pysid)
			{
				if($pygrades['period'] == 0)
				{
				...
				else if($pygrades['period'] == 2)
					$pyres2[$pysid-1][$pygrades['mid'][$pyrix]] = $pygrades['result'][$pyrix];
				else if($pygrades['period'] == 3)
					$pyres3[$pysid-1][$pygrades['mid'][$pyrix]] = $pygrades['result'][$pyrix];
			}
		}
  }
  ...
  foreach($students['sid'] AS $six => $sid)
  {
	foreach($subjects['mid'] AS $mid)
	{
	  ...
	  { // First, see if we need to put a result here from the previous year as vrijstelling
		if(isset($_GET['extractfromprevyear']) && isset($pyres[$sid][$mid]) && $pyres2[$sid][$mid] >= 6.0 && $pyres3[$sid][$mid] >= 6.0)
		{
  ...
  echo("<td><center>CKV</center></td><td><center>Uitslag</center></td></tr>");
