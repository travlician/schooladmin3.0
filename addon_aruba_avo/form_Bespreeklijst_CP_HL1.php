...
//
  ...
  $vakcats = array("Individu","Maatschappij","Taal & Communicatie","Kunst & Cultuur","Natuur","Wiskunde");
  /* LS */
  $vakhead["Individu"] = array("i&s","lo");
  ...
  $vakhead["Natuur"] = array("na","sk","bio","inf");
  $vakhead["Wiskunde"] = array("wiA","wiB","rek");
  $newan = array("ne","en","wi","asw","n&t");
  $ptvakken = array("i&s","lo","ec","m&o","ak","gs","ne","en","sp","pap","ckv","na","sk","bio","inf","wiA","wiB","rek");
  ...
  */
  ...
  if(isset($groups))
  {
    ...
    foreach($groups['gid'] AS $gix => $gid)
	{
	  ...
	  unset($subjdata);
	  unset($ckvinvalid);
	  $sdquery = "SELECT type, fullname, shortname, data FROM class LEFT JOIN subject USING(mid) LEFT JOIN ". $teachercode. " USING(tid) WHERE gid=". $gid. " AND type<>'meta'";
	  $sdquery .= " UNION SELECT type, fullname, shortname, '' FROM subject WHERE type='meta'";
	  $sdquery .= " UNION SELECT type, fullname, shortname, GROUP_CONCAT(DISTINCT data) FROM class LEFT JOIN subject USING(mid) LEFT JOIN ". $teachercode. " USING(tid) LEFT JOIN sgroup USING(gid) WHERE active=1 AND groupname LIKE 'HL%' AND groupname NOT LIKE 'CB%' GROUP BY mid";
	  $subjectdata = SA_loadquery($sdquery);
	  foreach($subjectdata['shortname'] AS $cix => $subjab)
	  {
	    if(!isset($subjdata[$subjab]))
		{
	  ...
	  if(isset($students))
	  {
		...
		while ($llnoffset < sizeof($students['sid']))
		{
		  ...
		  for($sx = 1; $sx <= $scnt; $sx++)
		  {
			...
			// Correction for HL1 only: if average of period 1 and 2 > period 0 then period 0 is averge of period 1 and 2 for ne
			foreach($stres AS $stix => $strs)
			{ // aka for each student
				foreach($strs AS $ssn => $strsn)
				{ // Aka for each subject
					if($ssn == "xne" && isset($strsn[1]) && isset($strsn[2]) && round(($strsn[1] + $strsn[2]) / 2.0,$ssn == "i&s" ? 1 : 0) > $strsn[0])
...
/*		  
		  ...
		  echo("<TR><TH class=headleft COLSPAN=2>Afw. uren</TH>");
		  for($sx = 1; $sx <= $scnt; $sx++)
		  {
		    // Get the absence hours data
		  ...
		  echo("<TR><TH class=headleft COLSPAN=2>Afw. dagen</TH>");
		  for($sx = 1; $sx <= $scnt; $sx++)
		  {
		    // Get the absence days data
			$absq = "SELECT SUM(IF(date >= '". $pdata[1]["sdate"]. "' AND date < '". $pdata[2]["sdate"]. "',1,0)) AS ap1";
			$absq .= ",SUM(IF(date >= '". $pdata[2]["sdate"]. "' AND date < '". $pdata[3]["sdate"]. "',1,0)) AS ap2";
			$absq .= ",SUM(IF(date >= '". $pdata[3]["sdate"]. "',1,0)) AS ap3";
			$absq .= " FROM absence WHERE sid=". $students['sid'][$llnoffset + $sx]. " AND (";
