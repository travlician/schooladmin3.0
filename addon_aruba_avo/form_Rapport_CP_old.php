...
//
  ...
  $groups = SA_loadquery("SELECT * FROM sgroup LEFT JOIN ". $teachercode. " ON(tid_mentor=tid) WHERE groupname LIKE '". $_SESSION['CurrentGroup']. "' ORDER BY groupname");
    
  if(isset($groups))
  {
    // First part of the page
    echo("<html><head><title>Rapport CP</title></head><body link=blue vlink=blue>");
    ...
    foreach($groups['gid'] AS $gix => $gid)
	{
	  ...
	  if(isset($students))
	  {
	    ...
	    echo("Geen studenten in deze groep");
  ...
  function stud_grades($sid,$schoolyear)
  {
    ...
    if(isset($teachercode))
      $sql_query = "SELECT class.mid,cid,shortname,fullname,". $teachercode. ".data AS `tcode` FROM subject inner join class using (mid) left join ". $teachercode. " USING(tid) WHERE show_sequence IS NOT NULL GROUP BY mid ORDER BY show_sequence";
    else
      $sql_query = "SELECT class.mid,cid,shortname,fullname FROM subject inner join class using (mid) WHERE show_sequence IS NOT NULL GROUP BY mid ORDER BY show_sequence";
...
//    $sql_query = "SELECT * FROM student inner join gradestore using (sid) where period='0' AND gradestore.year='" . $periods['year'][0] . "' AND student.sid=". $student->get_id();
	...
	$lj = explode("-",$_SESSION['CurrentGroup']);
   echo("<br><font size=+1><center>Leerjaar: " . $lj[0] . "</font><br>"); 
