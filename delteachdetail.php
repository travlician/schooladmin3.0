...
//
  ...
  { // O O, no result while there should be a table!
    echo($dtext['op_fail'] . " Could not find table name in teacher details table, check for spaces in table name");
    echo("<br><a href=manteacherdetails.php>" . $dtext['back_teachdetman'] . "</a>");
    SA_closeDB();
    exit;
  }
  ...
  $sql_query = "SELECT table_name,seq_no FROM teacher_details WHERE seq_no>'" . $old_seq_no . "'";
  $sql_result = mysql_query($sql_query,$userlink);
  ...
  // Now it's time to delete the entery from the teacher_details table
  $sql_query = "DELETE FROM teacher_details WHERE table_name='" . $table_name . "'";
  $sql_result = mysql_query($sql_query,$userlink);
  if(!$sql_result)
