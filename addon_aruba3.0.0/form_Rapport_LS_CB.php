...
//
  ...
  $vakhead["F. Wiskunde"] = array("WI","REK");
  ...
  if(isset($groups))
  {
    ...
    foreach($groups['gid'] AS $gix => $gid)
	{
      ...
      $sid = $students['sid'][1+$llnoffset];
		  ...
		  echo("<SPAN class=leftlabel>Ciclo Basico</SPAN><SPAN class=midfill1CB>&nbsp;". $groups['groupname'][$gix]. "</SPAN><B>Schooljaar: ". $schoolyear. "</B><BR>");
		  echo("<SPAN class=leftlabel>Leerling:</SPAN><SPAN class=studentname>&nbsp;". $students['firstname'][1+$llnoffset]. " ". $students['lastname'][1+$llnoffset]. "</SPAN><BR>");
		  echo("<SPAN class=leftlabel>Mentor:</SPAN><B> ". $groups['data'][$gix]. "</B><BR>");
		  echo("<SPAN class=leftlabel>Rapportage ". $repper. "</SPAN><SPAN class=schoolname>". $schoolname. "</SPAN><BR>");
		  echo("<SPAN class=subresultsheader>Rapportcijfers ". $repper. "</SPAN>");
		  ...
		  if(isset($detres))
			...
			{
			  if($detres['type'][$drix] == "OH")
			    $detresoh[$detres['mid'][$drix]][$tdid] = $detres['result'][$drix];
			  else if(substr($detres['type'][$drix],0,2) == "PW")
		  ...
		  foreach($ptvakken AS $subsj)
		  {
			...
			if(isset($detresoh[$subjdata[$subsj]['mid']]))
			{
			  foreach($detresoh[$subjdata[$subsj]['mid']] AS $ohres)
			    echo("<SPAN class=subresult>". ($ohres < 5.5 ? "<font color=red>" : ""). number_format(str_replace(" ","",str_replace("..",".",$ohres)),1,',','.'). ($ohres < 5.5 ? "</font>" : ""). "</SPAN>");
					//echo($ohres);
			}
			else
			  echo("&nbsp;");
			...
			if(isset($detrespw[$subjdata[$subsj]['mid']]))
			{
			  foreach($detrespw[$subjdata[$subsj]['mid']] AS $pwres)
			    echo("<SPAN class=subresult>". ($pwres < 5.5 ? "<font color=red>" : ""). number_format(str_replace(" ","",str_replace("..",".",$pwres)),1,',','.'). ($pwres < 5.5 ? "</font>" : ""). "</SPAN>");
					//echo($pwres);
			}
			else
			  echo("&nbsp;");
		  ...
		  if(isset($opmdata['lastmodifiedat'][0]))
		    ...
		    $opmdate = date("d-m-Y");
		  echo("<div class=opmbox><span class=opmleft>Opmerking:</span><span class=opmdate>DATUM: ". $opmdate. "</SPAN><BR>");
		  if(isset($opmdata['opmtext'][0]))
		    echo($opmdata['opmtext'][0]);		  
		  echo("</DIV></DIV>"); // end of remarkbox and leftpart of page div.
		  echo("<TABLE class=raptable><TR><TH class=rapmainhdr COLSPAN=3>Gemiddelde Rapport ". $repper. "</TH>");
		  echo("<TH class=rapperhdr rowspan=2>1ste R</TH>");
		  if($repper > 1)
		    echo("<TH class=rapperhdr rowspan=2>2de R</TH>");
		  if($repper > 2)
		    echo("<TH class=rapperhdr rowspan=2>3de R</TH><TH class=rapperhdr rowspan=2>EIND</TH>");
		  echo("</tr>");
		  echo("<tr><th class=leergebied>Leergebieden</th><th class=rapsubj colspan=2>Vakken</th></tr>");
		  ...
		  {
		    echo("<TR><TD class=cathead ROWSPAN=". sizeof($vakhead[$vk]). ">". $vk. "</TD>");
			//echo("</TR>");
		  ...
		  // Show points row
		  $sx=1;
		  echo("<TR><TD class=pointtd colspan=3>Aantal punten </TD>");
		  $percnt = $repper < 4 ? $repper : 3;
		  if($repper > 2)
		    $repper = 4;
		  for($p=0;$p<$percnt;$p++)
		  {
		  ...
		  echo("<TR><TD class=pointtd colspan=3>Aantal punten voor NEWAN vakken </TD>");
		  $percnt = $repper;
		  if($repper > 2)
		    $repper = 4;
		  for($p=0;$p<$percnt;$p++)
		  {
			...
			if(isset($stcaln[$sx][$pi]) && $stcaln[$sx][$pi] > 0)
			  echo(number_format($stcaln[$sx][$pi],1,",","."));
			else
			  echo("&nbsp;");
		    echo("</TD>");
		  } 
		  ...
		  $pi = ($repper > 3 ? 0 : $repper);
			if($stcalp[$sx][$pi] >= 90.0 && $stcalo[$sx][$pi] == 0 && $stcaln[$sx][$pi] >= 40.0)
				echo("VOLDOENDE"); //HV
			else if(($stcalp[$sx][$pi] >= 72.0 || ($stcalp[$sx][$pi] >= 71.0 && $stcalc[$sx][$pi] == 1)) && 
							$stcalo[$sx][$pi] <= 3 && $stcalc[$sx][$pi] <= 4 && $stcaln4[$sx][$pi] == 0)
				echo("VOLDOENDE"); // BV
			else if($stcalp[$sx][$pi] >= 66.0 && $stcalo[$sx][$pi] <= 4 && $stcalc[$sx][$pi] <= 5 && $stcaln4[$sx][$pi] == 0)
				echo("&nbsp;"); //BE
			else if(substr($groups['groupname'][$gix],0,1) == '1' && ($stcalp[$sx][$pi] < 65.0 || $stcalnp[$sx][$pi]  == 6))
				echo("ONVOLDOENDE"); // EPB
			else if(($stcalp[$sx][$pi] <= 65.0) ||  $stcalo[$sx][$pi] > 4 || $stcalc[$sx][$pi] > 5 || 
							$stcaln4[$sx][$pi] > 0)
				echo("ONVOLDOENDE"); // EPB
			else
				echo("&nbsp;"); // ?
