...
//
  ...
  /* MC */
  $vakhead["Taal & Communicatie"] = array("NED","ENG","SPA","PAP");
  $vakhead["Maatschappij"] = array("GES","AK");
  $vakhead["Exacte vakken"] = array("WIS","REK","NASK1","NASK2","BIO");
  $vakhead["Onderw. Onderst."] = array("EC","IK");
  $vakhead["Kunst & Cultuur"] = array("CKV");
  $vakhead["Individu"] = array("GD","LO");
  $ptvakken = array("LO","GES","AK","EC","PAP","NED","ENG","SPA","CKV","NASK1","NASK2","WIS","BIO","IK");
	  ...
	  if(isset($students))
	  {
		...
		while ($llnoffset < sizeof($students['sid']))
		{
				...
				foreach($ptvakken AS $vak)
				{
					for($p=0;$p<4;$p++)
					{
						if(isset($stres[$sx+$llnoffset][$vak][$p]))
						{
							...
							{
								$stcalo[$sx][$p]++;
								$stcalt[$sx][$p] += 6 - round($stres[$sx+$llnoffset][$vak][$p],0);
							}			    
						}
					}
				}
					...
					foreach($pvaks AS $vak)
					{
						for($p=0;$p<4;$p++)
						{
							if(isset($stres[$sx+$llnoffset][$vak][$p]))
							{
								...
								{
									$stcalvo[$sx][$p]++;
									$stcalvt[$sx][$p] += 6 - round($stres[$sx+$llnoffset][$vak][$p],0);
		    ...
		    for($p=0;$p<4;$p++)
				{
					$pi = ($p + 1) % 4;
					echo("<TD>");
					if(isset($stcalt[$sx][$pi]) && $stcalp[$sx][$pi] > 0)
						echo($stcalt[$sx][$pi]);
					else
						echo("&nbsp");
					echo("</TD>");
		    ...
		    for($p=0;$p<4;$p++)
				{
					$pi = ($p + 1) % 4;
					echo("<TD>");
					if($stcalp[$sx][$pi] >= 83 && $stcalo[$sx][$pi] <= 4 && $stcalt[$sx][$pi] <= 5 && $stcalv4[$sx][$pi] == 0 && $stcalvo[$sx][$pi] <= 2 && $stcalvt[$sx][$pi] <= 2)
						echo("BEV");
					else if($stcalp[$sx][$pi] <= 83 && $stcalp[$sx][$pi] >= 78 && $stcalo[$sx][$pi] <= 4 && $stcalt[$sx][$pi] <= 6 && $stcalv4[$sx][$pi] == 0 && $stcalvo[$sx][$pi] <= 2 && $stcalvt[$sx][$pi] <= 2)
						echo("BES");
					else
						echo("ONV");
					echo("</TD>");
		  ...
		  echo("</TR>");
			...
			// Debugging: show the subjects in the subjectpackage
