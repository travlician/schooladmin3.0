...
//
  ...
  $offsubjects = array(1 => "Ne","En","I&S","Sp","Pa","Wi-A","Wi-B","Na","Sk","Bio","Ec","M&O","Ak","Gs","Pfw","CKV");
  // AH & CP alternates
  $altsubjects = array("Ne"=>1,"En"=>2,"I&S"=>3,"Sp"=>4,"Wi-A"=>6,"Wi-B"=>7,"Na"=>8,"Sk"=>9,"Bio"=>10,"Ec"=>11,"M&O"=>12,"Ak"=>13,"Gs"=>14,"Pfw"=>15,"CKV"=>16,
                       "ne"=>1,"en"=>2,"i&s"=>3,"sp"=>4,"pap"=>5,"wiA"=>6,"wiB"=>7,"na"=>8,"naBB"=>8,"sk"=>9,"skBB"=>9,"bio"=>10,"ec"=>11,"m&o"=>12,"ak"=>13,"gs"=>14,"pfw"=>15,"ckv"=>16);
  ...
  $squery .= " LEFT JOIN s_exnr USING(sid) LEFT JOIN s_package USING(sid) LEFT JOIN sgroup USING(gid) WHERE active=1 AND s_exnr.data IS NOT NULL AND s_exnr.data > '0' AND groupname='ExamHavo' GROUP BY sid ORDER BY s_exnr.data";
  ...
  $ahxqr = SA_loadquery("SELECT sid,mid,xstatus FROM ahxdata WHERE year='". $schoolyear. "'");
  if(isset($ahxqr['sid']))
    foreach($ahxqr['sid'] AS $xdix => $xsid)
	  $beh[$xsid][$ahxqr['mid'][$xdix]] = str_replace(".",",",$ahxqr['xstatus'][$xdix]);
  ...
  echo("<html><head><title>Formulier EX. 2-HSE</title></head><body link=blue vlink=blue bgcolor=#E0E0FF>");
