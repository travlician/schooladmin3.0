...
//
  ...
  $altsubjects = array("NAT4"=>6, "GES4"=>11, "AK4"=>10, "EC4"=>9, "BIO4"=>8, "SCH4"=>7, "WIS4"=>5, "PAP4"=>4, "SPA4"=>3, "ENG4"=>2, "NED4"=>1,
					   ...
					   "NA"=>6, "EC"=>9, "EM & O"=>9, "CKV"=>12, "Ckv"=>12, "NED"=>1, "ENG"=>2, "SPA"=>3, "PAP"=>4, "WIS"=>5, "NASK1" => 6,"NS2"=>7, "NASK2" => 7, "BIO"=>8, "GES"=>11, "CKV"=>12, "bio"=>8);
  ...
  foreach($offsubjects AS $osix => $sjname)
  {
		foreach($subjectsqr['shortname'] AS $sbix => $subsn)
		{
			if(isset($altsubjects[$subsn]) && $altsubjects[$subsn] == $osix)
			{
				...
				if($osix ==7)
					echo("<BR>Not a match for ". $subsn);
		}
  }
	foreach($subjects['shortname'] AS $sbix => $subsn)
    echo("<BR>". $subsn. " -> ". $subjects['mid'][$sbix])  ;
	exit;
  ...
  $squery = "SELECT sid,lastname,firstname,gid,s_exnr.data AS exnr,packagename,extrasubject FROM student LEFT JOIN sgrouplink USING(sid)";
