...
//
  ...
  $slinkq = "SELECT sseqid,CONCAT(IF(separate=1,'@',''),IF(highlight=1 AND separate=0,'*',''),IF(fullname IS NOT NULL,fullname,'-')) AS subjname,shortname
  ...
  { // Edit sequence of subjects for student schoolcard in stead of showing the schoolcards.
	...
	echo("<table><TR><TH>#</TH><TH>Vak</TH><TH>Afscheiden</TH><TH>Vet</TH></TR>");
...
/* 	Paragraaf over de gegevens van de ouders:	*/
	...
	// Hier komt echt een zooitje, voor burgerlijke staat wordo ingevuld : D,G,O,S,W,Gehuwd,Gescheiden,ongehuwd,samenwonend of Weduwe.
		...
		$klasyears = inputclassbase::load_query("SELECT year,groupname FROM testresult LEFT JOIN testdef USING(tdid) LEFT JOIN class USING(cid)
		                                         LEFT JOIN sgroup USING(gid) WHERE sid=". $student->get_id(). " GROUP BY year,groupname DESC ORDER BY year");
		...
		foreach($grades AS $jaar => $dummy)
		{
		  if(isset($yeardata[$jaar]['klas']))
		    echo("<SPAN class=bojaar>". substr($yeardata[$jaar]['klas'],(isset($yearpositioningroup) ? $yearpositioningroup-1 : 0),1). "</SPAN>;");
		  else
		...
		echo("<table class = TblPropRap>
				...
				print_houding("Inzet","Inzet",$tablepos,$houding);
				print_houding("Gedrag","Gedr",$tablepos,$houding);
				print_houding("Regels","Regels",$tablepos,$houding);
				print_houding("Huiswerk","HWerk",$tablepos,$houding);
				print_houding("Concentratie","Conc",$tablepos,$houding);
				print_houding("Capaciteit","Cap",$tablepos,$houding);
				print_houding("Werkverzorging","Wrkvz",$tablepos,$houding);
				print_houding("Tempo","Tempo",$tablepos,$houding);
				echo("
