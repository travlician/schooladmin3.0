...
//
  ...
  $offsubjects = array(1 => "Ne","En","Sp","Wa","Wb","Na","Sk","Bi","Mo","Ec","Gs","Ak","CKV");
  $noexam = array("Gs","Ak");
  $altsubjects = array("Ne"=>1,"En"=>2,"Sp"=>3,"Wi-A"=>4,"Wi-B"=>5,"Na"=>6,"Sk"=>7,"Bio"=>8,"Ec"=>10,"M&O"=>9,"Ak"=>12,"Gs"=>11,"Pfw"=>14,"CKV"=>13,
                       "ne"=>1,"en"=>2,"sp"=>3,"wiA"=>4,"wiB"=>5,"na"=>6,"sk"=>7,"bio"=>8,"ec"=>10,"m&o"=>9,"ak"=>12,"gs"=>11,"pws"=>14,"ckv"=>13);
  ...
  function print_head()
  {
	...
	echo("<td class=fatbotgreen rowspan=2><center>TOT<BR>TV1</td><td colspan=2 class=nobotgreen><center>Uitslag 1ste tijdvak</center></td><td colspan=6 class=herhead><b><center>Herex</center></b></td>");
	echo("<td class=nobotgreen colspan=2><center><b>Einduitslag</b></center></td></tr>");
	
	echo("<tr><td class=fatbot>Ex-no</td><td class=fatbotgreenleft>Geb. datum/Land</td><td class=fatbotgreen>&nbsp;</td><td class=fatbot>&nbsp</td><td class=subjheadergem>Combi</td><td class=fatbot>&nbsp;</td>");
	$skipsubj = 0;
	foreach($subjects['shortname'] AS $sn)
	  if($skipsubj < 2)
	  {
	...
	echo("<td class=fatbotgreen>TOT</td>");
  ...
  function print_foot()
  {
    global $pageno;
	echo("</TABLE>");
	echo("<p class=footer><span class=formdata>EX 5-V (". date('n/j/Y; g:i A'). ") Bron: myschoolresults.com</span><span class=pagenr>Volgnummer ". $pageno++. "</span>");
  ...
  $squery = "SELECT sid,lastname,firstname,s_exnr.data AS exnr,packagename,extrasubject,s_ASGender.data AS gender,";
  ...
  $squery .= " WHERE active=1 AND s_exnr.data IS NOT NULL AND s_exnr.data > '0' AND groupname='ExamVwo' ORDER BY s_exnr.data";
  ...
  echo("<html><head><title>Formulier EX. 5-V</title></head><body link=blue vlink=blue onfload=\"window.print();setTimeout('window.close();',10000);\">");
  ...
  // Student listing
  {
    ...
    if($subjcount > 7 && !$passedtv1)
    {
	  ...
	  foreach($pkcandidates AS $pkname => $dummy)
	  {
	    ...
	    if(!(($certconditions && $newsubjcount >= 7 && $newnegpoints == 0) ||
	       ...
	       (!$certconditions && $newsubjcount >= 7 && $newtotpoints >= ($newsubjcount * 6) && $newnegpoints <= 2)))
		{
	      unset($pkcandidates[$pkname]);
		}
	  }
	...
	foreach($subjects['mid'] AS $sbix => $mid)
	{
	  ...
	  {  // Do this repeated subject
		...
		if(isset($endarray[$sid][$mid]) && isset($soarray[$sid][$mid]) && isset($exarray[$sid][$mid]))
        {
		  ...
		  if($endarray[$studs['sid'][$six]][$mid] < 6)
		    $negpoints += 6 - $endarray[$studs['sid'][$six]][$mid];
		}
	    else
  	      echo("X");
		echo("</td>");
	...
	if(!isset($exr[$sid]))
	{
	  ...
	  if($passedtv2)
	  {
		...
		else
		{
		  if(isset($pkcandidates) && count($pkcandidates >= 1))
		  {
			...
			foreach($pkcandidates AS $pkname => $pkscnt)
			  $pkct .= $pkname. ",";
			echo("Geslaagd (D7)<BR>". substr($pkct,0,-1));
		  }
		}
	  }
	  else
	  {
	    ...
	    foreach($subjects['mid'] AS $six => $mid)
	    {
	      ...
	      if(isset($endarray[$sid][$mid]) && $endarray[$sid][$mid] >= 6.0)
		    $reward += 50.0;
	    }
	  }
	  echo("</b></td>");
	...
	{ // Finance reg: remove reward
	  mysql_query("DELETE FROM s_beloning_1_". substr($schoolyear,0,4). " WHERE sid=". $sid, $userlink);
	}
	else
	{
	  mysql_query("REPLACE INTO s_beloning_1_". substr($schoolyear,0,4). " (sid,data) VALUES(". $sid. ",". $reward. ")", $userlink);
	}
	unset($reward);
  ...
  echo("</table>");
  echo("<p class=summarysign>Eerste tijdvak : percentage ". round(($passTV1m + $passTV1v) / ($totm + $totv) * 100,0). " %");
  ...
  echo("<p class=summarysign>Tweede tijdvak : percentage ". round(($passTV2m + $passTV2v) / ($totm + $totv) * 100,0). " %");
  echo("<BR><BR>Aruba<BR><BR><BR><BR>De Voorzitter: _________________________________________");
  echo("<BR><BR><BR><BR>De Secretaris: _________________________________________</p>");
