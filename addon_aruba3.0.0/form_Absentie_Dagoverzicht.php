...
//
  ...
  require_once("student.php");
	...
	$fieldlist = array("lastname","firstname","groupname","description","date","time","explanation","authorized");
  ...
  // Get a list of the absence
	$absence = inputclassbase::load_query("SELECT * FROM absence LEFT JOIN absencereasons USING(aid) LEFT JOIN student USING(sid) LEFT JOIN sgrouplink USING(sid) LEFT JOIN sgroup USING(gid) WHERE date>='". $absdate. "' AND date<='". $absdate2. "' AND (groupname LIKE '". $groepfilter. "') GROUP BY asid");
	echo("<html><head><title>Absentie dagoverzicht</title></head><body link=blue vlink=blue>");
	echo("<H1>Absentie dagoverzicht ");
