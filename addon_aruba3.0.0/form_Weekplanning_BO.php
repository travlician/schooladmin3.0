...
require_once("schooladminfunctions.php");
  ...
  if(isset($_POST['fieldid']))
  {
    ...
    if($_POST['fieldid'] == "dumyfield")
		...
		//if(substr($_POST['fieldid'],0,8) == "PWADDPLT") // Add for debug
		//	echo("PW");
    ...
    include("inputlib/procinput.php");
		...
		if(substr($_POST['fieldid'],0,8) == "PWADDPLT")
		{
			...
			$pwdataqr = inputclassbase::load_query("SELECT bo_weekplan_data.*,type FROM bo_weekplan_data LEFT JOIN subject ON(subject=mid) WHERE planid=". $plankey);
			if($pwdataqr['type'][0]=="meta")
				$pwmid=$pwdataqr['pwmid'][0];
			else
				$pwmid=$pwdataqr['subject'][0];
			// Get the corresponding class
			$cidqr = inputclassbase::load_query("SELECT * FROM `class` WHERE tid=". $pwdataqr['tid'][0]. " AND mid=". $pwmid);
			...
			{ 
				...
				{ // Ok, so we got a tdid, need to change the desription
					...
					echo("Modified tdid ". $tdidqr['tdid'][0]);
				}
				else
				{ // So, it's going to be a new item, we need short_desc, description, date, type, period, cid, year and week
					...
					$short_desc = "PW". substr($pwdataqr['datum'][0],8,2). substr($pwdataqr['datum'][0],5,2);
  ...
  echo(mysql_error());
	...
	// We added a new field, so check if it's there already and create it if not.
	$haspwmidcol= SA_loadquery("SHOW COLUMNS FROM bo_weekplan_data LIKE 'pwmid'");
	if(!isset($haspwmidcol['Field']))
	{ // Need to add the column
		mysql_query("ALTER TABLE bo_weekplan_data ADD COLUMN pwmid INT(11)");
		echo(mysql_error());
	}
  ...
  $slist = SA_loadquery("SELECT mid,fullname,type FROM subject");
  foreach($slist['mid'] AS $six => $smid)
	{
    $subjectlist[$smid] = $slist['fullname'][$six];
    $subjecttype[$smid] = $slist['type'][$six];
	}
  ...
  echo("<form method=post action=form_Weekplanning_BO.php id=teacherselect name=teacherselect><p class=selectors>");
  echo("<a href=form_Weekplanning_BO.php?pweek=1>Voorgaande week <img border=0 src='PNG/arrow_back.png'></a>");
  echo(" <a href=form_Weekplanning_BO.php?nweek=1><img border=0 src='PNG/arrow_next.png'> Volgende week</a> ");
  ...
  echo("</select> <a href=form_Weekplanning_BO.php?print=0><img src='PNG/file.png' alt=Afdrukken title=Afdrukken border=0></a></form></p>");
...
//    $mayedit = false;
  ...
  echo("<form name=delayedrefresh id=delayedrefresh method=post action=form_Weekplanning_BO.php></form>");
  // And it's JavaScript
  ...
  function get_roosteritem($gid,$day,$starttime)
  {
	...
	{
		global $userlink,$curyear,$negindex,$subjecttype;
		if(!$firstentry)
			echo("<HR>");
		...
		if($rd['text'] != "PAUZE" && $rd['text'] != "")
		{
			...
			{ // New entry for editing
				...
				echo("<BR>Proefwerk:");
				if($subjecttype[$rd['mid']] == "meta")
				{ // For meta subjects we add a field to select the sub-subject for tests
					$pwmidfield = new inputclass_listfield("pwmid". $planid,"SELECT '' AS id,'' AS tekst UNION SELECT mid,shortname FROM subject WHERE type='sub' AND meta_subject=". $rd['mid'],$userlink,"pwmid","bo_weekplan_data",$planid,"planid",NULL,NULL);
					if($mayedit)
						$pwmidfield->echo_html();
					else
						echo($pwmidfld->__toString());
				}
				echo("<BR>");
