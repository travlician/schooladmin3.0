...
// 19 jun 2017: for civil state family should we use numbers in stead of text if school = Pius X
if(strpos($announcement,"Pius") !== FALSE)
	$civilstatetrans = array("Gehuwd"=>1,"Ongehuwd"=>2,"Gescheiden"=>4,"Weduwe"=>6,"Samenwonend"=>5);
...
//$newstuds = SA_loadquery("SELECT * FROM nieuwe_inschrijving LEFT JOIN nieuwe_registratie USING(regid)");
if(isset($newstuds['regid']))
{
  ...
  foreach($newstuds['regid'] AS $six => $regid)
  {
	...
	// Set registration year and class
	mysql_query("REPLACE INTO s_ASRegistrationClass (sid,data) VALUES(". $sid. ",\"". $newstuds['KlasPlaatsing'][$six]. "\")",$userlink);
	echo(mysql_error($userlink));	
	mysql_query("INSERT INTO s_ASRegistrationDate (sid,data) VALUES(". $sid. ",\"". date("Y"). "\")",$userlink);
	echo(mysql_error($userlink));	
	...
	foreach($transtable AS $fieldname => $tablename)
	{
	  ...
	  {
			// 19 jun 2017: from issue with PX: default home language is 1 (Papiamento)
			if($tablename == "s_ASHomeLanguage")
				if($newstuds['LangHome'][$six] == '')
					$newstuds['LangHome'][$six] = 1;
			// 19 jun 2017: from issue with PX: default gender is m
			if($tablename == "s_ASGender")
				if($newstuds['Mankind'][$six] == '')
					$newstuds['Mankind'][$six] = 'm';
			// 19 jun 2017: civil state family needs to be translated if school is Pius X
			if(isset($civilstatetrans) && $fieldname == "EstCivilFamily" && isset($civilstatetrans[$newstuds['EstCivilFamily'][$six]]))
				$newstuds['EstCivilFamily'][$six] = $civilstatetrans[$newstuds['EstCivilFamily'][$six]];
