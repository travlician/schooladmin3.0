...
//
  ...
  $lday = date("j",mktime(0,0,0,date("n")+1,0,date("Y")));
  $daynames = array( 1 => "M","D","W","D","V");
  for($dt=1; $dt<= $lday; $dt++)
  {
    $wday = date("N",mktime(0,0,0,date("n"),$dt,date("Y")));
    if($wday < 6)
	  $dtab[$dt] = $daynames[$wday];
  }
  $dispmonth = date("n",mktime(0,0,0,date("n")+1,0,date("Y")));
  $dispyear = date("Y",mktime(0,0,0,date("n")+1,0,date("Y")));
  ...
  $abscats = SA_loadquery("SELECT IF(LENGTH(name) < 5,name,CONCAT(SUBSTR(name,1,4))) AS abscat,acid FROM absencecats ORDER BY acid");
  // Convert to something more usefull
  foreach($abscats['acid'] AS $aix => $acid)
    $acat[$acid] = $abscats['abscat'][$aix];
  
  // Get the absence
  $aq = "SELECT asid,sid,CONCAT(SUBSTR(name,1,1),COUNT(asid)) AS indicator,CONCAT(DAY(date),'-',MONTH(date)) AS arday,DAY(date) AS aday, explanation, firstname,lastname,acid, COUNT(asid) AS abscnt";
  $aq .= " FROM absence LEFT JOIN absencereasons USING(aid) ";
  $aq .= " LEFT JOIN absencecats USING(acid) LEFT JOIN sgrouplink USING(sid) LEFT JOIN sgroup USING(gid) LEFT JOIN student USING(sid)";
  $aq .= " WHERE active=1 AND YEAR(date)=". $dispyear. " AND MONTH(date)=". $dispmonth. " AND groupname='". $_SESSION['CurrentGroup']. "' GROUP BY sid,date,acid ORDER BY date";
  ...
  {
    if(isset($absence[$absr['sid'][$aix]][$absr['aday'][$aix]]))
      $absence[$absr['sid'][$aix]][$absr['aday'][$aix]] .= $absr['indicator'][$aix];
	else
      $absence[$absr['sid'][$aix]][$absr['aday'][$aix]] = $absr['indicator'][$aix];
    $abscatc[$absr['sid'][$aix]][$absr['aday'][$aix]][$absr['acid'][$aix]] = $absr['abscnt'][$aix];
	if($absr['explanation'][$aix] != "")
	{
	  $remark[$aix] = $absr['arday'][$aix]. " ". $absr['firstname'][$aix]. " ". $absr['lastname'][$aix]. " : ". $absr['explanation'][$aix];
	  $lastremark = $aix;
	  $remcount++;
	}
  }
  ...
  if(isset($studs))
  {
	...
	foreach($studs['sid'] AS $six => $sid)
	{
	  ...
	  foreach($dtab AS $dn => $adt)
	  {
		...
		{
		  echo("<center>". $absence[$sid][$dn]. "</center>");
		  foreach($acat AS $acid => $catn)
		  {
		    $ind = substr($catn,0,1);
			if(isset($abscatc[$sid][$dn][$acid]))
			{
		      $cur[$acid] += $abscatc[$sid][$dn][$acid];
		      $totD[$dn]+= $abscatc[$sid][$dn][$acid];
			}
          }			
	    }
		else
