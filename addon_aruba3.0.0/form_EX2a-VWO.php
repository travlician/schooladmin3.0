...
//
  ...
  function print_head($subjectindex)
  {
   ...
   echo("<p style='margin-top:-15px;'>LIJST VAN CIJFERS, bedoeld in de artikelen 21, lid 1, 29, lid 3 en 67, lid 4 van het Landsbesluit eindexamens dagscholen VWO, HAVO, MAVO,");
	...
	{ // We do need to show teacher, group and #students!
	  ...
	  // Get the group name
	  $gnqr = SA_loadquery("SELECT groupname FROM sgroup WHERE active=1 AND groupname LIKE '%vwo%' AND gid=". $gid);
  ...
  $groups = SA_loadquery("SELECT groupname,gid FROM sgroup WHERE active=1 AND groupname LIKE '%vwo%' ORDER BY groupname");
...
{ // Show selection options
  ...
  echo("<FORM METHOD=POST ACTION='form_EX2a-VWO.php'>");
