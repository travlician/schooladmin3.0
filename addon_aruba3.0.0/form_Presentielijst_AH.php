...
//
  ...
  function print_head($year, $place, $schoolyear, $subject, $teacher, $groupname, $time = NULL)
  {
    ...
    echo("<table class=studlist><TR><TH class=firstcol>nr.</TH><TH class=othercol>Naam:</TH><TH class=othercol>Handtekening:</TH></TR>");
  }
...
{ 
  ...
  foreach($_POST AS $pix => $pvl)
  {
	...
	{ // Generate list for group with gid given
      ...
      $student = SA_loadquery("SELECT lastname,data FROM student LEFT JOIN sgrouplink USING(sid) LEFT JOIN s_roepnaam USING(sid) WHERE gid=". $gid. " GROUP BY sid ORDER BY lastname,data");
	  ...
	  foreach($subjects['shortname'] AS $suix => $sname)
	  {
		...
		{
		  echo("<TR class=studrow><TD>". ($snr < 10 ? "0". $snr++ : $snr++). "</TD><TD>". $sln. ", ". $student['data'][$six]. "</TD><TD>&nbsp</TD></TR>");
		}
...
{ // No print posted, must show filter and sort fields
  ...
  // Show the filters
  // Year layer
  echo("<FORM METHOD=POST ACTION='form_Presentielijst_AH.php' name=filter>");
  ...
  echo("<FORM METHOD=POST ACTION='form_Presentielijst_AH.php'>");
