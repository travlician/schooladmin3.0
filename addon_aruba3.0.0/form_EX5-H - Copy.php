...
//
  ...
  function print_head()
  {
	...
	echo("<table class=ex5table border=0><tr><td class=nobot>&nbsp;</td><td class=lastname>Achternaam</td><td class=firstnamehead>Voornamen</td><td class=fatbotgreen rowspan=2>v<BR>m</td><td class=fatbotgreen rowspan=2><center>pro-<BR>fiel</center></td><td colspan=6 class=gemdeel><b>Gemeenschappelijk deel</td>");
	$skipsubj=0;
  ...
  function print_foot()
  {
    global $pageno;
	echo("</TABLE>");
	echo("<p class=footer><span class=formdata>EX 5-H (". date('n/j/Y; g:i A'). ")</span><span class=pagenr>Volgnummer ". $pageno++. "</span>");
  ...
  // Student listing
  {
	...
	if(!isset($exr[$sid]))
	{
	  ...
	  if($passedtv2)
	  {
		...
		mysql_query("REPLACE INTO s_beloning_1_". $yrpf. " (sid,data) VALUES (". $sid. ",". ($insgqr['data'][1] - 50.0). ")", $userlink);
	  }
	  else
	  {
        echo("Afgewezen");
