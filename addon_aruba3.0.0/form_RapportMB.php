...
foreach($lllist AS $stud)
{
	if($stud != NULL)
	{
		...
		show_result("<span style='float: right; padding-right: 100px;'>Gemiddelde</span>Taal","ne",true,false,$curyear==1);
		...
		else
		{
			...
			show_result("AVIniveau","AVI",false,false,false,true,true);
		}
		echo("<tr><td class=tablespace colspan=4></td></tr>");
		...
		{ // Need to show only main
			show_result("Rekenen","re",true,false,$curyear==1,false);			
		}
		else
		{ // Need to show the subs as well
			...
			show_result("<span style='float: right; padding-right: 100px;'>Gemiddelde</span>Rekenen","re",true,false); //V2
...
function show_result($subjfull,$subjid,$normalsubj,$showletters,$showletters12=false,$noproc=false,$subfinal=false,$maini=false)
{
	global $stres;
	if($normalsubj)
	{
