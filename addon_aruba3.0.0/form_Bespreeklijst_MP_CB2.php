...
//
  ...
  //$vakhead["Maatschappij"] = array("ASW3","ASW1","ASW2");
	// Aangepast 22 jun ivm wijziging ASW indeling
  $vakhead["Maatschappij"] = array("ASW2");
  $vakhead["Taal & Communicatie"] = array("PAP","NED","ENG","SPA");
  $vakhead["Kunst & Cultuur"] = array("CKV");
  $vakhead["Natuur"] = array("N&T");
  $vakhead["Wiskunde"] = array("WIS","REK");
  $vakhead["Onderw. Onderst."] = array("IK");
	// Aangepast 22 jun ivm wijziging ASW indeling
  //$newan = array("NED","ENG","WIS","ASW3","N&T");
  //$newanp = array("NED","ENG","WIS","ASW3","N&T","PV");
  $newan = array("NED","ENG","WIS","ASW2","N&T");
  $newanp = array("NED","ENG","WIS","ASW2","N&T","PV");
	$new = array("NED","ENG","WIS");
	// Aangepast 22 jun ivm wijziging ASW indeling
  //$ptvakken = array("KGL","PV","LO","ASW3","PAP","NED","ENG","SPA","CKVB","N&T","WIS","IK");
  $ptvakken = array("KGL","PV","LO","ASW2","PAP","NED","ENG","SPA","CKV","N&T","WIS","IK");
		...
		while ($llnoffset < sizeof($students['sid']))
		{
		  ...
		  for($sx = 1; $sx <= $scnt; $sx++)
		  {
		    for($p=0;$p<4;$p++)
			{
							$pi = ($p + 1) % 4;
							echo("<TD>");
							if($stcalp[$sx][$pi] >= 90 && $stcalo[$sx][$pi] == 0 && $stcaln[$sx][$pi] >= 40)
								echo("HV");
							else if($stcalp[$sx][$pi] >= 72 && $stcalo[$sx][$pi] <= 3 && $stcalc[$sx][$pi] <= 4 && $stcals4[$sx][$p] == 0)
								echo("MV");
							else if($stcalp[$sx][$pi] == 71 && $stcalo[$sx][$pi] == 1 && $stcalc[$sx][$pi] == 1)
								echo("MV");
							else if($stcalp[$sx][$pi] >= 66 && $stcalp[$sx][$pi] <= 72 && $stcalo[$sx][$pi] <= 4 && $stcalc[$sx][$pi] <= 4 && $stcals5[$sx][$pi] == 0)
								echo("BES");
							else if($stcalp[$sx][$pi] < 65 || $stcalt[$sx][$pi] == 6)
								echo("EPB");
							else if($stcalp[$sx][$pi] < 66)
								echo("DOU");
							else
							{					
								echo("?");
								/*echo(":". $stcals5[$sx][$p]. ",". $stcals4[$sx][$p]. ",". $stcalt[$sx][$p]);
								if($stcals5[$sx][$p] > 0)
								{
									echo("{");
									foreach($new AS $vak)
										echo($vak. "=". $stres[$sx+$llnoffset][$vak][$pi]);
									echo("}");
								} */
							}
							echo("</TD>");
		  ...
		  echo("</TABLE>");
		  echo("<P class=footer>Advies codes: HV=HAVO, MV=MAVO bevorderen, BES=Bespreekgeval, DOU=Doubleren/EPB, EPB=EPB, ?=Niet vermeld in afspraken DO</P>");
