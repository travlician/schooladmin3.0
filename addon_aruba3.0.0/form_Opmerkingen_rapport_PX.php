...
//
  ...
  $yrsel = "SELECT '' AS id, '' AS tekst UNION SELECT 'OVER','OVER' UNION SELECT 'NIET OVER','NIET OVER' UNION SELECT 'O.W.L.','O.W.L.' UNION SELECT 'S.V.','S.V.' UNION SELECT 'V.B.Z.','V.B.Z.'";
  // Get a list of all applicable students
  $students = SA_loadquery("SELECT CONCAT(lastname,', ',firstname) AS name,sid FROM student LEFT JOIN sgrouplink USING(sid) LEFT JOIN sgroup USING(gid) LEFT JOIN s_ASGender USING(sid) WHERE active=1 AND sgroup.groupname = '$CurrentGroup' ORDER BY data,name");
  ...
  echo("<form method=post action=form_Opmerkingen_rapport_PX.php>" . $dtext['Group_Cap'] . " <select name=NewGroup>");
  ...
  foreach($students['sid'] AS $six => $sid)
  {
	...
	else
	{
	  ...
	  $adfield = new inputclass_listfield("yrfield". (--$negix),"SELECT '' AS id, '' AS tekst UNION SELECT 'HAVO','HAVO' UNION SELECT 'MAVO','MAVO' UNION SELECT 'EPB','EPB'",$userlink,"advice","bo_jaarresult_data",$negix,"resultid","","hdprogpage.php");
