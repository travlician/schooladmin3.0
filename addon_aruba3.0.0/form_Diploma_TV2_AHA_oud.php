...
//
  ...
  function stud_grades($student,$schoolyear,$group)
  {
	...
	// Check if this is a repeating candidate, only repeaters are printed.
	unset($rptqr);
	$rptqr = inputclassbase::load_query("SELECT shortname,xstatus FROM ex45data LEFT JOIN subject USING(mid) 
	                                     WHERE sid=". $student->get_id(). " AND year='". $schoolyear. "' AND 
										 xstatus < 5 AND xstatus > 0 AND xstatus IS NOT NULL AND mid>0");
	...
	if(!$passed || !isset($rptqr['xstatus']))
	{
	  //echo($student->get_lastname(). " Not passed (". $certconditions. ",". $subjcount. ",". $negpoints. ",". $coreshort. ",". $totpoints. ",". isset($rptqr['xstatus']). ")<BR>");
		//if(isset($certs))
		//	foreach($certs AS $ckey => $cval)
		//		echo("Cert: ". $ckey. "=". $cval. "<BR>");
	  return;
	}
	//echo("[". $passed. ",". $certconditions. ",". $subjcount. ",". $negpoints. ",". $coreshort. ",". $totpoints. ",". $fullfail. ",". $fails. ",". $choicesubfail. "]<BR>");
	...
	echo("<P class=diplomadate>". $_POST['rdate']. "</p>");
