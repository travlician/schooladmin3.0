...
//
  ...
  $vakhead["Maatschappij"] = array("ASW","ASW CB1","ASW CB", "ASW EC");
		...
		while ($llnoffset < sizeof($students['sid']))
		{
		  ...
		  for($sx = 1; $sx <= $scnt; $sx++)
		  {
		    for($p=0;$p<4;$p++)
			{
			  $pi = ($p + 1) % 4;
			  echo("<TD>");
			  if($stcalp[$sx][$pi] >= 100.0 && $stcalo[$sx][$pi] == 0 && $stcaln[$sx][$pi] >= 40.0)
			    echo("HV");
			  else if(($stcalp[$sx][$pi] >= 78.0 || ($stcalp[$sx][$pi] >= 77.0 && $stcalc[$sx][$pi] == 1)) && $stcalo[$sx][$pi] <= 3 && $stcalc[$sx][$pi] <= 4 && $stcaln4[$sx][$pi] == 0)
			    echo("BV");
			  else if($stcalp[$sx][$pi] >= 72.0 && $stcalo[$sx][$pi] <= 4 && $stcalc[$sx][$pi] <= 4 && $stcaln5[$sx][$pi] == 0)
			    echo("BE");
