...
{ 
  ...
  //echo("<html><head><title>Ontbrekende Toetslijst Avond HAVO</title></head><body link=blue vlink=blue bgcolor=#FFFFE0 onload=\"window.print();setTimeout('window.close();',10000);\">");
  echo("<html><head><title>Ontbrekende Toetslijst Avond HAVO</title></head><body link=blue vlink=blue bgcolor=#FFFFE0 >");
  ...
  foreach($_POST AS $pix => $pvl)
  {
	...
	{ // Generate list for group with gid given
      ...
      $student = SA_loadquery("SELECT lastname,data,sid FROM student LEFT JOIN sgrouplink USING(sid) LEFT JOIN s_roepnaam USING(sid) WHERE gid=". $gid. " GROUP BY sid ORDER BY lastname,data");
	  ...
	  foreach($subjects['shortname'] AS $suix => $sname)
	  {
		...
		$tcntq = "SELECT sgrouplink.sid, IF(tcnt1 IS NULL,0,tcnt1) AS tcnt FROM sgrouplink LEFT JOIN 
		           (SELECT sid,COUNT(tdid) AS tcnt1 FROM testresult LEFT JOIN testdef USING(tdid) LEFT JOIN class USING(cid)
					...
					ON(sgrouplink.sid=t1.sid) WHERE sgrouplink.gid=". $gid. " GROUP BY sid HAVING tcnt < 2";
...
{ // No print posted, must show filter and sort fields
  ...
  // Show the filters
  // Year layer
  echo("<FORM METHOD=POST ACTION='form_OntbrekendeToetslijst_AH.php' name=filter>");
  ...
  echo("<FORM METHOD=POST ACTION='form_OntbrekendeToetslijst_AH.php'>");
  ...
  if(isset($groups['gid']))
  {
	...
	{
	  if(substr($groups['groupname'][$gix],0,3) == "OS3" || substr($groups['groupname'][$gix],0,3) == "SN3" || substr($groups['groupname'][$gix],0,3) == "VWO")
	    $checked = "";
	  else
	    $checked = "CHECKED";
