...
//
  ...
  $vakcats = array("Cijfers");
  /* MH */
  $vakhead["Cijfers"] = array("ne","en","sp","wa","bio","ec","m&o","ak","gs");
  $ptvakken = array("ne","en","sp","wa","bio","ec","m&o","ak","gs");
  ...
  */
  $afwezigreden = array(1,2,3,4,5,11);
  $telaatreden = array(6,7,8,9,10);
  $groepfilter = "CB_' OR groupname LIKE 'H_' OR groupname LIKE 'HL_' OR groupname LIKE 'V_";
  $llnperpage = 8;
...
//	  $sdquery .= " UNION SELECT type, fullname, shortname, data FROM subject LEFT JOIN (SELECT meta_subject,data FROM subject LEFT JOIN class USING(mid) LEFT JOIN ". $teachercode. " USING(tid) WHERE gid=". $gid. " AND type='sub' GROUP BY mid) AS t1 ON (subject.mid=t1.meta_subject) WHERE type='meta'";
	  $sdquery = "SELECT type, fullname, shortname, data FROM class LEFT JOIN subject USING(mid) LEFT JOIN ". $teachercode. " USING(tid)";
	  ...
	  if(isset($students))
	  {
		...
		while ($llnoffset < sizeof($students['sid']))
		{
		  ...
		  echo("<TR><TH COLSPAN=2>Vakken</TH>");
		  for($sx = 1; $sx <= $scnt; $sx++)
		  {
		  ...
		  for($sx = 1; $sx <= $scnt; $sx++)
		  {
		    ...
		    $sres = SA_loadquery("SELECT period, result, shortname FROM gradestore LEFT JOIN subject USING(mid) 
								  LEFT JOIN (SELECT mid,cid FROM sgrouplink LEFT JOIN class USING(gid) WHERE sid=". $students['sid'][$llnoffset+$sx]. ") AS t1 USING(mid)
			                      WHERE sid=". $students['sid'][$llnoffset+$sx]. " AND year=\"". $schoolyear. "\" AND cid IS NOT NULL");
			if(isset($sres))
		  ...
		  foreach($vakcats AS $vk)
		  {
			...
			{
			  echo("<TR><TD COLSPAN=2>". $subjdata[$vkn]["fullname"]. "</TD>");
		      for($sx = 1; $sx <= $scnt; $sx++)
		      {
			    echo("<TD>");
		  ...
		  echo("<TR><TH class=headleft COLSPAN=2>Resultaat</TH>");
		  for($sx = 1; $sx <= $scnt; $sx++)
		  {
		    for($p=0;$p<4;$p++)
			{
			  ...
			  echo("<TD>");
			  /*
			  if($stcalp[$sx][$pi] >= 35 && $stcalo[$sx][$pi] <= 1 && $stcalc[$sx][$pi] <= 1)
			    echo("V");
			  else if($stcalp[$sx][$pi] >= 36 && $stcalc[$sx][$pi] <= 2)
			    echo("B");
			  else
			    echo("O");
			  */
		  ...
		  echo("</TABLE>");
		  //echo("<P class=footer>Resultaat codes: V=Voldoende, B=Bespreekgeval, O=Onvoldoende / Onvolledig</P>");
  ...
  else echo("<html>Geen groepen gevonden");
