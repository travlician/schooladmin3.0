<?php
  ...
  $mainsubjectabbrevs = array( 1 => "vk","en","sp","lo","hv","te","mu","go","ne","le","re","sc","ak","gs","kdn","ma");
  $subsubjects["Ned"] = array(1 => "&nbsp;&nbsp;&nbsp;<i>Dictee</i>", "&nbsp;&nbsp;&nbsp;<i>Taaloefening</i>","&nbsp;&nbsp;&nbsp;<i>Tekst</i>");
  $subsubjectabbrevs["Ned"] = array(1 => "dc","to","tk");
...
//  $rres = SA_loadquery("SELECT sid,data FROM s_ASRefferedTo LEFT JOIN student USING(sid) LEFT JOIN sgrouplink USING(sid) WHERE gid=". $gid);
  ...
  echo(">");
  echo '<LINK rel="stylesheet" type="text/css" href="style_fcrapport.css" title="style1">';
  
  foreach($student_array['sid'] AS $si => $sidx)
  {
    // Header: Logo left and school info text right
	echo("<P class=titlerow>RAPPORT</p>");
	echo("<IMG  class=schoollogo src=schoollogo.png border=0>");
	...
	echo("<P class=schoolinfo>Fatima College<BR>Patiastraat 19<BR>Tel.: 582-2037<BR>&nbsp;</p>");
...
/*	echo("</TR>");
	...
	// remarks on report
	echo("<P class=opmhead>Opmerkingen eerste rapport:</p>");
	echo("<P class=opmrap>". (isset($rapopms[$sidx][1]) ? $rapopms[$sidx][1] : "&nbsp;"). "</p>");
	echo("<P class=opmhead>Opmerkingen tweede rapport:</p>");
	echo("<P class=opmrap>". (isset($rapopms[$sidx][2]) ? $rapopms[$sidx][2] : "&nbsp;"). "</p>");
	echo("<P class=opmhead>Opmerkingen derde rapport:</p>");
	echo("<P class=opmrap>". (isset($rapopms[$sidx][3]) ? $rapopms[$sidx][3] : "&nbsp;"). "</p>");
	...
	echo("<TR><TD>Hoofd:</TD><TD>Franklin Richardson</TD><TD>________________________________________</TD></TR>");
	...
	if(isset($yrresult[$sidx]))
	{
	  if($yrresult[$sidx] == "OVER")
	    ...
	    echo("Niet bevorderd; verwezen naar ");
    }
	echo("&nbsp;");
