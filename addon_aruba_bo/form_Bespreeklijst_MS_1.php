...
//
  ...
  $vakcats = array("Hoofdvakken","Andere vakken");
  /* MC */
  $vakhead["Hoofdvakken"] = array("Ned","Taal","Tekst","Dictee","Rek","Tech. L","Avi");
  $vakhead["Andere vakken"] = array("Schr","Verkeer","EN","Spa","LO","HV","Tekenen","Muziek","Godsd");
  $ptvakken = array("Ned","Tech. L","Rek");
    ...
    foreach($groups['gid'] AS $gix => $gid)
		{
			...
			$students = SA_loadquery("SELECT student.* FROM student LEFT JOIN sgrouplink USING(sid) LEFT JOIN s_ASGender USING(sid) WHERE gid=". $gid. " ORDER BY data,lastname,firstname");

			// Get a list of period dates
			$pquery = "SELECT id AS period, startdate AS sdate, enddate AS edate FROM period ORDER BY id";
			...
			if(isset($students))
			{
				...
				while ($llnoffset < sizeof($students['sid']))
				{
					...
					for($sx = 1; $sx <= $scnt; $sx++)
					{
						echo("<TH COLSPAN=3 class=studnamecel>". ($sx+$llnoffset). ". ". $students['lastname'][$sx+$llnoffset]. "<BR><SPAN class=stfirstname>". $students['firstname'][$sx+$llnoffset]. "</SPAN></TH>");
					}
					echo("</TR>");
					...
					for($sx = 1; $sx <= $scnt; $sx++)
					{
						echo("<TH class=leftB>1</TH><TH>2</TH><TH>3</TH>");
					}
					echo("</TR>");
					...
					{
						echo("<TR><TD class=cathead COLSPAN=1>". $vk. "</TD>");
						for($sx = 1; $sx <= $scnt; $sx++)
						{
							echo("<TD COLSPAN=3 class=leftB>&nbsp;</TD>");
						}
						echo("</TR>");
						foreach($vakhead[$vk] AS $vkn)
						{
							...
							{
								echo("<TR><TD". ($subjdata[$vkn]['type'] == "meta" ?  ' class=metahead' : ($subjdata[$vkn]['type'] == "sub" ? ' class=subhead' : '')). ">". 
											$subjdata[$vkn]["fullname"]. "</TD>");
								for($sx = 1; $sx <= $scnt; $sx++)
								{
									echo("<TD class=rescolB>");
									if(isset($stres[$llnoffset+$sx][$vkn][1]))
										echo(colored($stres[$llnoffset+$sx][$vkn][1],false,true,$subjdata[$vkn]["type"] == "meta",$vkn=='ac'));
									else
										echo("&nbsp;");
									echo("</TD>");
									...
									if(isset($stres[$llnoffset+$sx][$vkn][0]))
										echo(colored($stres[$llnoffset+$sx][$vkn][0],false,false,$subjdata[$vkn]["type"] == "meta"));
									else
										echo("&nbsp;");
									echo("</TD>"); */
								}
						...
						if($vk == "Hoofdvakken")
						{
							...
							for($sx = 1; $sx <= $scnt; $sx++)
							{
								...
								for($p=0;$p<4;$p++)
									if(isset($stres[$sx+$llnoffset]["Tech. L"][$p]) && $stres[$sx+$llnoffset]["Tech. L"][$p] >= 5.5)
										$stcall[$sx][$p] = true;
							}
							
							// Show calculated results
							echo("<TR><TD><B>TOTAAL PUNTEN</b> (>= 17)</TD>");
							for($sx = 1; $sx <= $scnt; $sx++)
							{
								for($p=0;$p<3;$p++)
								{
					...
					foreach($aspects AS $aspkey => $aspname)
					{
						...
						for($sx = 1; $sx <= $scnt; $sx++)
						{
							...
							if(isset($behave[$aspsid][$aspkey][1]))
								echo("<TD class=leftB><center>". $behave[$aspsid][$aspkey][1]. "</center></td>");
							else
								echo("<TD class=leftB>&nbsp;</td>");
							if(isset($behave[$aspsid][$aspkey][2]))
								echo("<TD><center>". $behave[$aspsid][$aspkey][2]. "</center></td>");
							else
								echo("<TD>&nbsp;</td>");
							if(isset($behave[$aspsid][$aspkey][3]))
								echo("<TD><center>". $behave[$aspsid][$aspkey][3]. "</center></td>");
							else
								echo("<TD>&nbsp;</td>");
							//echo("<TD>&nbsp;</td>");								
						}
					...
					for($sx = 1; $sx <= $scnt; $sx++)
					{
						...
						if(isset($absdat))
						{
							echo("<TD class=leftB>". $absdat["ap1"][1]. "/". $absdat["apu1"][1]. "</TD>
										<TD>". $absdat["ap2"][1]. "/". $absdat["apu2"][1]. "</TD>
										<TD>". $absdat["ap3"][1].  "/". $absdat["apu3"][1]."</TD>");
						}
						else
								echo("<TD COLSPAN=3 class=leftB>&nbsp;</TD>");
						unset($absdat);
					...
					for($sx = 1; $sx <= $scnt; $sx++)
					{
						for($p=0;$p<3;$p++)
						{
							...
							if($pi==1)
								...
								echo("<TD>");
							if($pi == 1 || !isset($stcalp[$sx][$pi]))
								echo("&nbsp;");
							else if($stcalp[$sx][$pi] >= 17 && $stcalc[$sx][$pi] < 2 && $stcall[$sx][$pi])
								echo("Bev");
							else
								echo("NB");
							echo("</TD>");
