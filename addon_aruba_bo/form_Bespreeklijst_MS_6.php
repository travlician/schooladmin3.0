...
//
  ...
  /* MC */
  $vakhead["Hoofdvakken"] = array("Ned","Taal","Tekst","Dictee","Rek");
	$vakhead["Zaakvakken"] = array("AK","Gesch","KdN");
  $vakhead["Andere vakken"] = array("Tech. L","Avi","Schr","Verkeer","EN","Spa","LO","HV","Tekenen","Muziek","Godsd","Maats.L");
  $ptvakken = array("Ned","Rek");
    ...
    foreach($groups['gid'] AS $gix => $gid)
		{
			...
			if(isset($students))
			{
				...
				while ($llnoffset < sizeof($students['sid']))
				{
					...
					for($sx = 1; $sx <= $scnt; $sx++)
					{
						echo("<TH COLSPAN=4 class=studnamecel>". ($sx+$llnoffset). ". ". $students['lastname'][$sx+$llnoffset]. "<BR><SPAN class=stfirstname>". $students['firstname'][$sx+$llnoffset]. "</SPAN></TH>");
					}
					echo("</TR>");
					...
					foreach($vakcats AS $vk)
					{
						...
						foreach($vakhead[$vk] AS $vkn)
						{
							if(isset($subjdata[$vkn]))
							{
								...
								if(isset($stres[$llnoffset+$sx][$vkn][0]))
									echo(colored($stres[$llnoffset+$sx][$vkn][0],false,false,$subjdata[$vkn]["type"] == "meta",$vkn=='ac'));
								else
									echo("&nbsp;");
								echo("</TD>");
									}
						...
						if($vk == "Hoofdvakken")
						{
							...
							echo("<TR><TD><B>TOTAAL PUNTEN</b></TD>");
							for($sx = 1; $sx <= $scnt; $sx++)
							{
								for($p=0;$p<4;$p++)
								{
						...
						if($vk == "Zaakvakken")
						{
							...
							for($sx = 1; $sx <= $scnt; $sx++)
							{
								...
								foreach($vakhead["Zaakvakken"] AS $vak)
								{
									for($p=0;$p<4;$p++)
									{
										if(isset($stres[$sx+$llnoffset][$vak][$p]))
										{
											$stcalzp[$sx][$p] += round($stres[$sx+$llnoffset][$vak][$p],0);
											if($stres[$sx+$llnoffset][$vak][$p] < 5.5)
											{
												$stcalzo[$sx][$p]++;
							...
							/* echo("<TR><TD><B>ONVOLDOENDES</b></TD>");
							for($sx = 1; $sx <= $scnt; $sx++)
							{
								for($p=0;$p<4;$p++)
								{
					...
					for($sx = 1; $sx <= $scnt; $sx++)
					{
						...
						if(isset($absdat))
						{
							echo("<TD class=leftB>". $absdat["ap1"][1]. "/". $absdat["apu1"][1]. "</TD>
										...
										<TD class=leftE>". ($absdat['ap1'][1]+$absdat['ap2'][1]+$absdat['ap3'][1]). "/". ($absdat['apu1'][1]+$absdat['apu2'][1]+$absdat['apu3'][1]). "</td>");
						}
						else
								echo("<TD COLSPAN=4 class=leftB>&nbsp;</TD>");
						unset($absdat);
					...
					echo("<TR><TH class=headleft>Advies</TH>");
					for($sx = 1; $sx <= $scnt; $sx++)
					{
						for($p=0;$p<4;$p++)
						{
					...
					echo("</TABLE>");
					echo("<P class=footer>Advies codes: Vanwege onduidelijkheid in het SKOA document \"Bevorderingsnormen\" wordt geen advies weergegeven!</P>");
