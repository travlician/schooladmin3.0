...
//
  ...
  $students = SA_loadquery("SELECT CONCAT(lastname,', ',firstname) AS name,firstname, lastname, sid FROM student LEFT JOIN sgrouplink USING(sid) LEFT JOIN sgroup USING(gid) LEFT JOIN s_ASGender USING(sid) WHERE active=1 AND sgroup.groupname = '$CurrentGroup' ORDER BY data,name");
  ...
  echo("<form method=post action=form_Houding_invoer_MS.php>" . $dtext['Group_Cap'] . " <select name=NewGroup>");
