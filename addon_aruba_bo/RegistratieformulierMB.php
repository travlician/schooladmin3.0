...
// Dit inschrijfformulier is gemaakt voor de Avond Havo
 ...
 // $login_qualify = 'ACT';
  ...
  echo(mysql_error());
	...
	mysql_query("ALTER TABLE inschrijvingMB2 ADD COLUMN exMBstudent TEXT DEFAULT NULL AFTER workSPCOA", $userlink);
  ...
  { // Convert registration code to rid (record in registation table)
    $rid=base64_decode(urldecode($_GET['regcode']));
		$typeyrqr = inputclassbase::load_query("SELECT schooltype,year FROM inschrijvingMB2 WHERE rid=". $rid);
...
<?
  ...
  if(isset($_POST['rid']))
  {
    echo("<FIELDSET style='background-color: #FFFFFF'><LEGEND>Inschrijving Mon Plaisir Basis</LEGEND>");
		...
		echo("<BR><LABEL>Ouder(s) oud-leerling MP basis/kleuter:</LABEL> ");
		$lnfld = new inputclass_listfield("exMBstudent","SELECT '' AS id,'' AS tekst UNION SELECT 'ja' AS id, 'Ja/Si' AS tekst UNION SELECT 'nee' AS id, 'Nee/No' AS tekst",NULL,"exMBstudent","inschrijvingMB2",$rid,"rid",NULL,"inschrijfMBhandler.php");
		$lnfld->echo_html();
