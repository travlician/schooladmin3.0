...
//
  ...
  $cq = "SELECT sid,mid,result FROM gradestore LEFT JOIN period ON(period.id=gradestore.period) LEFT JOIN student USING(sid) LEFT JOIN sgrouplink USING(sid) LEFT JOIN sgroup USING(gid)";
  ...
  foreach($student_array['sid'] AS $si => $sidx)
  {
	...
	echo("<BR><label>Klasse-leerkracht:</label><B>". $teacher['firstname'][1]. " ". $teacher['lastname'][1]. "</B>");
