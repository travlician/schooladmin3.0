...
//
  ...
  $subjlinks = array("@NEDERLANDSE TAAL" => "ne", "Dictee" => "dc", "Taaloefeningen" => "to", "Tekst/Begr. Lezen" => "tk", "Taalverkennen" => "tv", "Woordenschat" => "ws", "Spelling" => "spe", "schrijven (stellen)" => "ss", "Spreken en luisteren" => "sl", "-endnl" => "",
                     "*LEZEN (technisch)" => "le", "AVI-niveau" => "an", "-endle" => "",
					 "*REKENEN" => "re", "Getallen/bewerking" => "gb", "Verh./Breuken/Procent." => "br", "Meten/Meetk/Tijd/Geld" => "mm", 
					 "Tabellen/Grafieken" => "tg", "Minimumtoets" => "mt", "Projecttoets" => "pt", "Basistoets" => "bt", "-endre" => "",
					 "*AARDRIJKSKUNDE" => "ak", "-endak" => "", "*GESCHIEDENIS" => "gs", "-endgs" => "", "*KENNIS DER NATUUR" => "kdn", "-endmain" => "",
					 "Godsdient" => "go", "Schrijven" => "sc", "Verkeer" => "vk", "Engels" => "en", "Spaans" => "sp", "Papiamento" => "pa", "Maatschappijleer" => "ml",
					 "Bewegingsonderwijs" => "lo", "Handvaardigheid" => "hv", "Tekenen" => "te", "Muziekale vorming" => "mu", "-endall" => "");
...
//	De opmaak van de "minitabel rechtsboven:
		echo("<div class = rechtsboven> <table class = minitabel>
											...
											<tr><td>S.K.O.A. nr.: ". $student->get_student_detail("ASSKOANumber"). "</td></tr>
...
/*																 
	...
	$student->get_student_detail("s_ASAddress");
...
/* 	Paragraaf over de gegevens van de leerling:	*/
		...
		echo("<P class = LLdata>
				...
				Geb. Plaats: <span class = GebPlaatsLL>". $student->get_student_detail("s_ASBirthPlace"). "</span><br></P>");
...
/* 				Tel./ mob. staat op een aparte regel, los van de andere tekst, sinds 22-10-2013 niet meer!	*/
		echo("<div class = LLdata>
				...
				$student->get_student_detail("s_ASPhoneHomeParent2"). "<br>
				<span class = LinkerMarge2>Tel. / Mob.: ". $student->get_student_detail("s_ASPhoneHomeParent1"). "<br>
...
/* 	Paragraaf over de gegevens van de ouders:	*/
	...
	// Hier komt echt een zooitje, voor burgerlijke staat wordo ingevuld : D,G,O,S,W,Gehuwd,Gescheiden,ongehuwd,samenwonend of Weduwe.
		...
		echo("<tr><td colspan=2>Verzorger / voogd: <SPAN class = VoogdLL>". $student->get_student_detail("s_ASResponsablePers"). "</SPAN>
				Adres: <SPAN class = AdresvoogdLL>". $student->get_student_detail("s_ASAddressGuardian"). "</SPAN>
				Tel.: ". $student->get_student_detail("s_ASPhoneGuardian"). "</td></tr>");
		echo("</table>");
		...
		echo("<table class = GegDatInschrijving>
				...
				Reden:<SPAN class = DatumUitschrLL>". $student->get_student_detail("s_ASReasonDeregistration"). "</SPAN>
					Naar: ". $student->get_student_detail("s_ASFlowThrough_vo"). "<br>
				&nbsp;</tr>
			</table>");

		echo("<br>");	
		echo("Advies hoofd: <SPAN class=advieshoofd>". $student->get_student_detail("s_ASAdviceHead"). "</SPAN><SPAN class=wensspacer>&nbsp;</SPAN>");
		echo("Wens ouders: <SPAN class=wensouders>". $student->get_student_detail("s_ASWishParents"). "</SPAN>");
		echo("<br><br>");	
		echo("Schoolcarri&egrave;re K.O.: <u>&nbsp;". $student->get_student_detail("s_ASYearsKO"). " </u> jr.<SPAN class=jarenspacer>&nbsp;</SPAN>B.O.: ");
		...
		echo("<table class = TblPropRap>
				...
				<tr class = KleurRij><td>Rapport</td>");
				for($col=1;$col<9;$col++)
				{
				  echo("<td class = rapport1Header>R-1</td><td class=rapport2Header>R-2</td><td class=rapport3Header>R-3</td>");
				  if($col > 3) // Alleen kolom 4 t/m 8 hebben eindrapport
				    echo("<td class = rapportEHeader>E</td>");
				}
				echo("</tr>
...
echo("<br><br><br>");
  ...
  function print_grades($naam,$code,$tablepos,$grades)
  {
    ...
    if(substr($naam,0,1) == "*")
      ...
      echo("<tr class=fillrowcijferlijst><td style='height: 8px;'></td>");
    else
      echo("<tr><td>". $naam. "</td>");	
	for($tp=1;$tp<=8;$tp++)
	{
	  if(substr($naam,0,1) == "-")
	    echo("<TD style='height: 4px;". ($tp==8 ? "border-right: 4px solid black;" : ""). "' class = DikkelijnLinksRC colspan=". ($tp>3 ? "4" : "3"). ">");
	  else
	  {
	    echo("<td class = DikkelijnLinksRC>");
