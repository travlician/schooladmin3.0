...
//
  ...
  /* MC */
  $vakhead["Hoofdvakken"] = array("ne","bl","to","dc","le","lb","vv","ac","an","re","bt","pt","mt");
  $vakhead["Andere vakken"] = array("sc","ak","gs","kdn","vk","en","sp","lo","hv","te","mu","go","ct","tt");
  $vakhead["Taal & Communicatie"] = array("Pa","Ne","En","Sp");
  $ptvakken = array("ne","le","re");
    ...
    foreach($groups['gid'] AS $gix => $gid)
		{
			...
			if(isset($students))
			{
				...
				while ($llnoffset < sizeof($students['sid']))
				{
					...
					for($sx = 1; $sx <= $scnt; $sx++)
					{
						echo("<TH COLSPAN=3 class=studnamecel>". ($sx+$llnoffset). ". ". $students['lastname'][$sx+$llnoffset]. "<BR><SPAN class=stfirstname>". $students['firstname'][$sx+$llnoffset]. "<BR>". $students['lb'][$sx+$llnoffset]. "</SPAN></TH>");
					}
					echo("</TR>");
